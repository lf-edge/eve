...
# Used by get_pkg_build_yml (Makefile) and by get-deps via the exported
# EVE_PKG_BUILD_YML_<UPPERCASE> env var convention.
# To add another version-driven package, add a line here and export it.
EVE_PKG_BUILD_YML_zfs := $(ZFS_BUILD_YML)
export EVE_PKG_BUILD_YML_ZFS := $(ZFS_BUILD_YML)
...
FORCE_BUILD=
# Default: no dep-hash-triggered force rebuild. Overridden per-target in pkg-deps.mk.
DEPS_FORCE =
...
# Skip on clean/distclean to avoid triggering Dockerfile generation.
...
eve-%: pkg/%/Dockerfile $(LINUXKIT) $(RESCAN_DEPS)
	...
	$(eval LINUXKIT_FLAGS := $(if $(filter manifest,$(LINUXKIT_PKG_TARGET)),,$(FORCE_BUILD) $(DEPS_FORCE) $(LINUXKIT_DOCKER_LOAD) $(LINUXKIT_BUILD_PLATFORMS)))
	...
	@mkdir -p .gen-deps; \
	  _tag=$$($(LINUXKIT) pkg $(LINUXKIT_ORG_TARGET) show-tag --build-yml $(call get_pkg_build_yml,$*) pkg/$*); \
	  if [ "$$(cat .gen-deps/$*.hash 2>/dev/null)" != "$$_tag" ]; then \
	    printf '%s' "$$_tag" > .gen-deps/$*.hash; \
	  elif [ -n "$(DEPS_FORCE)" ]; then \
	    touch .gen-deps/$*.hash; \
	  fi
...
# Dependency graph: conditional on get-deps binary changes.
# Generates per-consumer hash file prereqs and DEPS_FORCE assignments.
pkg-deps.mk: $(GET_DEPS)
	$(QUIET)$(GET_DEPS) $(ROOTFS_GET_DEPS) -d .gen-deps -m $@

# Hash-update rule: runs get-deps -H which iterates all pkg/*/build.yml,
# computes linuxkit content tags via pkglib (pure git, no Docker), and writes
# .gen-deps/<pkg>.hash with write-if-changed semantics.
.gen-deps/%.hash: $(GET_DEPS)
	@mkdir -p .gen-deps
	$(QUIET)$(GET_DEPS) -d .gen-deps -H
