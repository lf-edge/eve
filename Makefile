...
# Per-package build yml overrides for version-driven packages.
# Used by get_pkg_build_yml via EVE_PKG_BUILD_YML_<lowercase-name>.
EVE_PKG_BUILD_YML_zfs := $(ZFS_BUILD_YML)
...
PARSE_PKGS=$(if $(strip $(EVE_HASH)),EVE_HASH=)$(EVE_HASH) DOCKER_ARCH_TAG=$(DOCKER_ARCH_TAG) KERNEL_TAG=$(KERNEL_TAG) \
    ...
    HASH_DIR=$(CURDIR)/.gen-deps ./tools/parse-pkgs.sh
...
GEN_HASH_DEPS_DIR=./tools/gen-hash-deps
GEN_HASH_DEPS=./tools/gen-hash-deps/gen-hash-deps
...
PKGS_riscv64=pkg/ipxe pkg/mkconf pkg/mkimage-iso-efi pkg/grub     \
	     ...
	     pkg/debug pkg/zfs pkg/dom0-ztools pkg/gpt-tools pkg/storage-init pkg/mkrootfs-squash \
...
# Package build targets and dep graph — generated by pkg update-hashes + gen-hash-deps.
# Switching ZARCH invalidates hash files (arch field mismatch), triggering rebuilds.
# When absent, make's -include mechanism triggers the bootstrap rule and re-reads.
-include .gen-deps/hash-deps.mk
...
clean:
	rm -rf $(DIST) images/out pkg-deps.mk .gen-deps
	rm -f $(GEN_HASH_DEPS)
...
$(GEN_HASH_DEPS): $(GEN_HASH_DEPS_DIR)/*.go
	$(MAKE) -C $(GEN_HASH_DEPS_DIR)
...
pkgs: RESCAN_DEPS=
pkgs: $(LINUXKIT) $(patsubst pkg/%,eve-%,$(PKGS)) $(LK_POSSIBLE_BUILD_ARG_TARGETS)
	@echo Done building packages
...
%-cache-export-docker-load: $(LINUXKIT) pkg/%
	$(eval IMAGE_TAG := $(shell $(LINUXKIT) pkg $(LINUXKIT_ORG_TARGET) show-tag --hash-dir .gen-deps --canonical pkg/$*))
	$(eval CACHE_CONTENT := $(shell $(LINUXKIT) cache ls 2>&1))
...
# Returns the build yml to use for a package, in priority order:
#   1. EVE_PKG_BUILD_YML_<name> override (version-driven packages, e.g. zfs)
#   2. HV=k variants (build-k[-dev].yml)
#   3. RSTATS=y variant (build-rstats.yml)
#   4. DEV=y variant (build-dev.yml)
#   5. build-<PLATFORM>.yml if it exists, else build.yml
get_pkg_build_yml = $(or $(EVE_PKG_BUILD_YML_$(subst -,_,$1)), \
...
# Step 1: compute all package hashes and the dep graph.
# update-hashes writes .gen-deps/*.hash with epoch-0 mtime for new/changed
# files (write-if-changed preserves mtime when nothing changed).
# gen-hash-deps reads the hash files and emits inter-package ordering rules.
# make's -include mechanism re-reads the Makefile after this rule fires.
.gen-deps/hash-deps.mk: $(wildcard pkg/*/build.yml) | $(LINUXKIT) $(GEN_HASH_DEPS) .gen-deps
	$(LINUXKIT) $(DASH_V) pkg update-hashes --hash-dir .gen-deps --arch $(ZARCH) \
	    $(foreach p,$(filter pkg/%,$(PKGS)),$p:$(strip $(call get_pkg_build_yml,$(notdir $p))))
	$(GEN_HASH_DEPS) -d .gen-deps -o $@

# update-hashes: refresh hashes and regenerate hash-deps.mk.
...
# Step 2: per-package build targets.
# Source file prereqs and inter-package deps from hash-deps.mk drive make's
# out-of-date detection.
# Recipe:
#   1. Recompute this package's tag via update-hashes (single-pkg, reads dep
#      tags from .gen-deps/).  Writes epoch-0 mtime if tag/arch/build-yml changed.
#   2. Build the Docker image via pkg build.  On success linuxkit touches the
#      hash file to set a real mtime, completing the build-stamp cycle.
.SECONDEXPANSION:
...
.gen-deps/%.hash: $$(shell find pkg/$$* \! -path '*/vendor/*' -type f 2>/dev/null) | .gen-deps $(LINUXKIT)
	...
	$(QUIET)$(LINUXKIT) $(DASH_V) pkg update-hashes --hash-dir .gen-deps --arch $(ZARCH) \
...
# the bare "pkg" dir entry that find produces when building PKGS_$(ZARCH).
$(filter pkg/%,$(PKGS)): pkg/%: eve-%
	@:
...
%-show-tag: .gen-deps/hash-deps.mk
	@$(LINUXKIT) pkg $(LINUXKIT_ORG_TARGET) show-tag --hash-dir .gen-deps --canonical pkg/$*
