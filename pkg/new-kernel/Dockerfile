...
ARG KERNEL_VERSION_aarch64=5.15.46
ARG KERNEL_VERSION_x86_64=5.15.46
ARG KERNEL_SOURCE
...
SHELL ["/bin/sh", "-c", "export KERNEL_SERIES=$(eval echo \\$KERNEL_VERSION_$(uname -m) | sed -e 's#^\\([^.]*\\)\\.\\([^.]*\\).*$#\\1.\\2.x#') ; /bin/sh -c \"$1\"", "-" ]
