...
func (a *MMAgent) startModemManager(setLogLevel bool) {
	...
	a.log.Noticef("Started ModemManager with PID: %d\n", a.mmProcess.Pid)
...
func (a *MMAgent) stopModemManager() {
	if a.mmProcess == nil {
		// This would be a bug in MMAgent.
	...
	// Before stopping ModemManager, close DBus client.
	...
	if err != nil && !errors.Is(err, os.ErrProcessDone) {
		...
		if err != nil && !errors.Is(err, os.ErrProcessDone) {
			a.log.Errorf("Failed to kill ModemManager process with PID %d: %v",
				a.mmProcess.Pid, err)
			return
		}
	}
	...
	select {
	// No need to update MMAgent touch file, mmStopTimeout < wdTouchPeriod
	case <-time.After(mmStopTimeout):
		a.log.Errorf("ModemManager process with PID %d did not exit after %v",
	...
	case err := <-done:
		if err != nil {
			...
			a.log.Noticef("Stopped ModemManager with PID: %d\n", a.mmProcess.Pid)
		}
	}
	a.mmProcess = nil
}
