...
// and whether the ext4 /persist filesystem can be shrunk to make room. The GPT
// is read with the same go-diskfs the resize itself drives, so the check and the
// resize cannot disagree about the layout; the ext4 superblock is parsed directly
// so no filesystem has to be mounted. Both work on a disk image or a real block
// device, mounted or not.
...
//
// gpt.Read falls back to the backup GPT at end-of-disk when the primary fails
// CRC validation, reporting it through Table.RecoveredFromBackup. The pre-flight
// refuses such a disk instead of planning from the backup: the kernel and the
// bootloader run off the primary, so a plan built from a newer backup would
// describe a layout the running system is not using. A primary that fails CRC
// also means an earlier partition-table commit was interrupted, which the resize
// step has to finish before any check of the result is meaningful.
func readGPT(path string, sectorSize int64) (parts []Partition, diskSize int64, err error) {
	// Read-only, so this is a plain O_RDONLY open: safe while the disk's other
	...
	// Seeking to the end gives the size of a regular file and of a block device
	// alike, unlike Stat on a block device.
	diskSize, err = b.Seek(0, io.SeekEnd)
	if err != nil {
		return nil, 0, err
	}

	// Only the logical sector size affects where the GPT structures are; the
	// physical size is carried on the parsed entries and unused here.
	table, err := gpt.Read(b, int(sectorSize), int(sectorSize))
	if err != nil {
		return nil, 0, fmt.Errorf("read GPT: %w", err)
	}
	if table.RecoveredFromBackup {
		return nil, 0, errors.New("primary GPT failed CRC validation and was recovered from the backup GPT; repair the partition table before resizing")
	}
