...
its vault key from the controller), `status/nim/DevicePortConfigList`,
`status/zedclient/OnboardingStatus*` (the saved device UUID, so `cmd/client` can
take its already-onboarded shortcut with no controller reachable) and
`status/vaultmgr/VaultConfig`. The `/persist/certs/` keys cannot be re-derived
once the filesystem is wiped — only the controller-trust anchor in `/config`
survives the shrink on its own — so losing them would cost the device its
attestation identity, hence the backup.

`status/vaultmgr/VaultConfig` records whether the vault key is the TPM key alone
or that key merged with a build-time constant (the pre-7.10.0 form). When the
file is absent `vaultmgr` infers the mode from whether `/persist/vault` exists,
which on an already-encrypted vault always yields the merged form — so a device
that created its vault TPM-key-only derives the wrong key. The TPM unseal
succeeds and only `fscrypt` refuses, so the failure reads as a broken seal while
the vault stays locked on that boot and every later one, leaving the device in
maintenance mode. Nothing else on the device can reconstruct the mode.
