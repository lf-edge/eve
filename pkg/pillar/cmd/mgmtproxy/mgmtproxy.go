...
const (
	...
	// already in NO_PROXY.
	//
	// Unlike the loopback listener, this one is reachable by every pod on
	// the node, not just CDI importer pods, so it also requires the
	// Proxy-Authorization token, applies the destination policy, and (since
	// cni0 is a shared L2 bridge, so a pod could otherwise ARP-spoof this
	// address to intercept that token) is served over TLS with a
	// self-signed cert importer pods trust via CDI's TrustedCAProxy (all in
	// podpolicy.go); the loopback listener needs none of that.
...
type mgmtProxyContext struct {
	...
	// clusterMu protects the fields below, separately from mu so it never
	// contends with the hot dial path's snapshot().
	clusterMu sync.RWMutex
	// clusterConfigSeen is true once EdgeNodeClusterConfig has arrived at
	// least once. See proxyIdentity.
	clusterConfigSeen bool
	// clustered mirrors EdgeNodeClusterConfig.Valid.
	clustered bool
	// clusterToken is the decrypted cluster join token from zedkube's
	// EdgeNodeClusterStatus.EncryptedClusterToken. Only set when clustered.
	clusterToken string
	...
	// authToken holds the cni0 proxy-auth token once loaded/generated by
	// startProxySecretsReconciler. Unset (nil) until then, which checkProxyAuth
	// treats as "not ready" and fails closed.
	authToken atomic.Value

	// tlsCert holds a *tls.Certificate for the cni0 listener once loaded by
	// startProxySecretsReconciler. Unset (nil) until then, which the listener's
	// tls.Config.GetCertificate treats as "not ready" and fails the
	// handshake.
	tlsCert atomic.Value

	// authTokenKey holds the cluster token (or "" for a standalone/random
	// value) that authToken/tlsCert currently correspond to.
	authTokenKey atomic.Value

	// authReady is set once startProxySecretsReconciler has successfully
	// applied a token/cert.
...
// proxyIdentity reports the cluster token to derive the cni0 token/cert
// from, and whether that's known yet.
func (ctx *mgmtProxyContext) proxyIdentity() (clusterToken string, ready bool) {
	ctx.clusterMu.RLock()
	defer ctx.clusterMu.RUnlock()
	if !ctx.clusterConfigSeen {
		return "", false
	}
	if !ctx.clustered {
		return "", true
...
func Run(ps *pubsub.PubSub, loggerArg *logrus.Logger, logArg *base.LogObject,
	...
	// Feeds proxyIdentity: EdgeNodeClusterConfig says standalone vs.
	// clustered; EdgeNodeClusterStatus carries the join token once clustered.
	...
	subEdgeNodeClusterStatus, err := ps.NewSubscription(pubsub.SubscriptionOptions{
		AgentName:     "zedkube",
		MyAgentName:   agentName,
		TopicImpl:     types.EdgeNodeClusterStatus{},
		Activate:      false,
		Ctx:           ctx,
		CreateHandler: handleEdgeNodeClusterStatusCreate,
		ModifyHandler: handleEdgeNodeClusterStatusModify,
		DeleteHandler: handleEdgeNodeClusterStatusDelete,
		WarningTime:   warningTime,
		ErrorTime:     errorTime,
	})
	if err != nil {
	...
	ctx.subEdgeNodeClusterStatus = subEdgeNodeClusterStatus
	subEdgeNodeClusterStatus.Activate()

	// Load (or, on first boot, generate) the cni0 proxy-auth token in the
	// background; checkProxyAuth fails closed until it's ready.
	ctx.startProxySecretsReconciler()

	server := &http.Server{
		Addr:        ListenAddr,
		Handler:     newProxyHandler(ctx, false),
	...
	//
	// TLS-wrapped (see CNI0ListenAddr): GetCertificate reads ctx.tlsCert
	// lazily, so this can start listening immediately without waiting for
	// startProxySecretsReconciler — any handshake attempted before the cert is
	// ready just fails, the same fail-closed behavior checkProxyAuth
	// already has for the token.
	go func() {
		server2 := &http.Server{
			Handler:     newProxyHandler(ctx, true),
			ReadTimeout: 30 * time.Second,
		}
		tlsConfig := &tls.Config{
			GetCertificate: func(*tls.ClientHelloInfo) (*tls.Certificate, error) {
				cert, ok := ctx.tlsCert.Load().(*tls.Certificate)
				if !ok {
					return nil, fmt.Errorf("mgmtproxy: cni0 TLS certificate not yet loaded")
				}
				return cert, nil
			},
		}
		for {
			...
			log.Noticef("mgmtproxy: cni0 listener on %s (TLS)", CNI0ListenAddr)
			if serveErr := server2.Serve(tls.NewListener(listener, tlsConfig)); serveErr != nil &&
				serveErr != http.ErrServerClosed {
	...
	for {
		...
		case change := <-subEdgeNodeClusterStatus.MsgChan():
			subEdgeNodeClusterStatus.ProcessChange(change)
		case <-publishTimer.C:
...
func handleEdgeNodeClusterConfigCreate(ctxArg interface{}, key string,
	statusArg interface{}) {
	handleEdgeNodeClusterConfigImpl(ctxArg, key, statusArg)
}
...
func handleEdgeNodeClusterConfigImpl(ctxArg interface{}, key string,
	...
	cfg := statusArg.(types.EdgeNodeClusterConfig)
	ctx.clusterMu.Lock()
	ctx.clusterConfigSeen = true
	ctx.clustered = cfg.Valid
	ctx.clusterMu.Unlock()
	log.Functionf("mgmtproxy: EdgeNodeClusterConfig: clustered=%v", cfg.Valid)
}

// handleEdgeNodeClusterConfigDelete reverts to "not yet known" rather than
// assuming standalone, so proxyIdentity fails closed.
func handleEdgeNodeClusterConfigDelete(ctxArg interface{}, key string, _ interface{}) {
	ctx := ctxArg.(*mgmtProxyContext)
	if key != "global" {
		return
	}
	ctx.clusterMu.Lock()
	ctx.clusterConfigSeen = false
	ctx.clustered = false
	ctx.clusterMu.Unlock()
}

func handleEdgeNodeClusterStatusCreate(ctxArg interface{}, key string,
	statusArg interface{}) {
	handleEdgeNodeClusterStatusImpl(ctxArg, key, statusArg)
}

func handleEdgeNodeClusterStatusModify(ctxArg interface{}, key string,
	statusArg interface{}, _ interface{}) {
	handleEdgeNodeClusterStatusImpl(ctxArg, key, statusArg)
}

func handleEdgeNodeClusterStatusImpl(ctxArg interface{}, key string,
	statusArg interface{}) {
	ctx := ctxArg.(*mgmtProxyContext)
	if key != "global" {
		return
	...
	ctx.clusterMu.Lock()
	// EncryptedClusterToken is decrypted plaintext despite the name.
	ctx.clusterToken = status.EncryptedClusterToken
	ctx.clusterMu.Unlock()
}

func handleEdgeNodeClusterStatusDelete(ctxArg interface{}, key string, _ interface{}) {
	ctx := ctxArg.(*mgmtProxyContext)
	if key != "global" {
		return
	}
