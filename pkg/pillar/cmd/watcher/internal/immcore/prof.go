...
// NOTE: Hierarchical span handling was removed; each Profile call is independent.

const profilePath = "/persist/memory-monitor/output/imm-profile.dat"
...
// Pass min as 0 for no threshold; every of 0 will be treated as 1.
func SetProfilingConfig(min time.Duration, every uint64) {
	atomic.StoreInt64(&profMinNS, min.Nanoseconds())
	if every == 0 {
		every = 1
	}
