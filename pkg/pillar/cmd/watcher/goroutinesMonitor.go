...
type GoroutineLeakDetectionParams struct {
	...
	//nolint:containedctx // We intentionally store a cancellable context to control a long-lived monitoring goroutine; scope is limited and not passed elsewhere.
...
func GoroutinesMonitor(ctx *watcherContext) {
	...
	thresholdInit, checkInterval, checkStatsForInit, keepStatsFor, cooldownInit := ctx.GRLDParams.Get()
	// Silence unused (we only need checkInterval & keepStatsFor for initial capacity here)
	_ = thresholdInit
	_ = checkStatsForInit
	_ = cooldownInit
