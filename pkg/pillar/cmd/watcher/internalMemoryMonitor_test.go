...
func TestCleanupOldCsvFiles(t *testing.T) {
	...
	for i := 0; i < 6; i++ {
		fileName := filepath.Join(types.MemoryMonitorOutputDir,
			"memory_usage.csv."+time.Now().Add(-time.Duration(6-i)*time.Hour).Format("20060102T150405")+".old")
		if err := os.WriteFile(fileName, content, 0644); err != nil {
			t.Fatalf("Failed to create test file: %v", err)
		}
	...
	var totalSize int64
	for _, file := range files {
		if filepath.Ext(file.Name()) == ".old" || filepath.Ext(file.Name()) == ".csv" {
			csvCount++
			info, err := file.Info()
			if err == nil {
				totalSize += info.Size()
...
func runImmExperiment(binPath, inputFile, outputFile string) ([]ScoreResult, error) {
	...
	output, err := cmd.CombinedOutput()
	if err != nil {
		return nil, fmt.Errorf("imm-experiment failed: %v\nOutput: %s", err, output)
	}
...
func parseScoresFromCSV(filename string) ([]ScoreResult, error) {
	...
	if len(lines) < 2 {
		return nil, fmt.Errorf("no data in output CSV")
	}
	...
	if scoreIdx < 0 || recentIdx < 0 || entireIdx < 0 {
		return nil, fmt.Errorf("missing required columns in output CSV")
	}
