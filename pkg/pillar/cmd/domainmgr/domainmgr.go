...
func doInactivate(ctx *domainContext, status *types.DomainStatus, impatient bool) {
	...
	// (UserActivate=true) AND the VMIRS identity is unchanged
	// (PurgeCounter matches status). The legitimate teardown paths —
	// handleDelete (impatient=true), a real user opt-out
	// (UserActivate=false), and a purge (PurgeCounter change → kubeName
	// change) — bypass this guard.
	if ctx.hvTypeKube && !impatient {
		if cfg := lookupDomainConfig(ctx, status.Key()); cfg != nil &&
			cfg.UserActivate && cfg.PurgeCounter == status.PurgeCounter {
			log.Noticef("doInactivate(%s) kubevirt: skipping Delete/Cleanup "+
				"(UserActivate=true, PurgeCounter unchanged); leaving VMIRS to cluster",
				status.Key())
			return
		}
	}
...
func handleModify(ctx *domainContext, key string,
	...
	if config.Activate && !status.Activated && status.State != types.BROKEN && !status.HasError() {
		...
		// (PurgeCounter increments), and the OLD VMIRS must be torn down before
		// the NEW one is created. Discriminate via PurgeCounter.
		purgeChanged := config.PurgeCounter != status.PurgeCounter
		if ctx.hvTypeKube && config.UserActivate && !purgeChanged {
			if status.HasError() {
				log.Noticef("handleModify(%s) kubevirt: clearing error without "+
					"Delete (UserActivate=true, PurgeCounter unchanged): %s",
					status.Key(), status.Error)
				status.ClearError()
				publishDomainStatus(ctx, status)
				changed = true
			...
			// PurgeCounter change in kubevirt mode: kubeName is changing,
			// tear down the OLD VMIRS now (impatient=true bypasses the
			// kubevirt cluster-trust guard in doInactivate).
			impatient := ctx.hvTypeKube && purgeChanged
			if impatient {
				log.Noticef("handleModify(%s) kubevirt: PurgeCounter %d -> %d; "+
					"tearing down old VMIRS for kubeName change",
					status.Key(), status.PurgeCounter, config.PurgeCounter)
			}
			doInactivate(ctx, status, impatient)
			updateStatusFromConfig(status, *config)
			// Advance PurgeCounter only after the old VMIRS was successfully
			// torn down (status.Activated cleared by doInactivate's tail).
			// Otherwise a retry tick must still see purgeChanged=true so it
