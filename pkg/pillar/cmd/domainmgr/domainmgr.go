...
func configToStatus(ctx *domainContext, config types.DomainConfig,
	...
	for i, dc := range config.DiskConfigList {
		...
		if status.IsContainer {
			...
			//location = persistRktDataDir + "/cas/blob/sha512/" + string(status.ContainerImageID[7:9]) + "/" + status.ContainerImageID
...
func DomainCreate(status *types.DomainStatus) (int, error) {
	...
	if status.IsContainer {
		...
		domainID, err = rktRun(status, status.ContainerImageID, filename)
	} else {
		// Use xl tool
...
func rktRun(status *types.DomainStatus, ContainerImageID string, xenCfgFilename string) (int, error) {
