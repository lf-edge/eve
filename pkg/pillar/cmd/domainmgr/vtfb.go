...
func fbUnbindAll() error {
	...
	for _, fb := range fbDrvs {
		...
		log.Noticef("Unbind framebuffer driver: %v\n", fb.name)
		if err := fb.unbind(); err != nil {
			log.Warnf("Fail to unbind framebuffer driver %v: %v", fb.name, err)
		}
	}
...
func fbBindAll() error {
	...
	for _, fb := range fbDrvs {
		...
		// don't care)
		log.Noticef("Bind framebuffer driver: %v\n", fb.name)
		if err := fb.bind(); err != nil {
			log.Warnf("Fail to bind framebuffer driver %v: %v", fb.name, err)
		}
	}
...
func getVTConsoles() ([]vtInfo, error) {
