...
func createVdiskVolume(ctx *volumemgrContext, status types.VolumeStatus,
	...
	filelocation := appRwVolumeName(status.VolumeID.String(),
		uint32(status.GenerationCounter), status.ContentFormat, false)
...
func createContainerVolume(ctx *volumemgrContext, status types.VolumeStatus,
	...
	created := false
	dirName := appRwVolumeName(status.VolumeID.String(),
		uint32(status.GenerationCounter), status.ContentFormat, true)
