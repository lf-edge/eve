...
func getRemainingDiskSpace(ctxPtr *volumemgrContext) (uint64, error) {
	...
	log.Noticef("XXX getRemainingDiskSpace device total %d used %d free %d, allowed %d reservedAppDiskUsage %d",
	...
	if allowedDeviceDiskSize < reservedAppDiskUsage {
		log.Noticef("XXX getRemainingDiskSpace: ZERO")
		return 0, nil
	} else {
		log.Noticef("XXX getRemainingDiskSpace: %d",
			allowedDeviceDiskSize-reservedAppDiskUsage)
