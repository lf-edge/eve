...
// VERIFIED <= State < LOADED -- the same role maybeSpaceAvailable plays for
// volumes. Called from every worker result handler, since a completed job is
// what frees the slot a refused submission needs; everything the pool can
