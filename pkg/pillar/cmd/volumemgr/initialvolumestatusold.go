	...
	for _, location := range locations {
		...
		if err != nil {
			log.Errorf("populateInitialOldVolumeStatus: Invalid UUIDStr(%s) in "+
				"filename (%s). err: %s. XXX Ignoring deleting the File",
...
func gcResetOldObjectsLastUse(ctx *volumemgrContext, dirName string) {
	log.Debugf("gcResetOldObjectsLastUse()")
	pub := ctx.publication(types.OldVolumeStatus{}, types.UnknownObj)
	items := pub.GetAll()
	for _, st := range items {
		status := st.(types.OldVolumeStatus)
		if status.RefCount == 0 {
			log.Infof("gcResetOldObjectsLastUse: reset %v LastUse to now", status.Key())
