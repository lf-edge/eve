...
type tpmSanityCheckError struct {
	err          error
	tpmErrorType int // TODO : add this to eve-api
}
...
func Run(ps *pubsub.PubSub, loggerArg *logrus.Logger, logArg *base.LogObject, arguments []string, baseDir string) int {
	...
	// what this means? shoud I set it?
	pubTpmStatus.ClearRestarted()
	...
	if check := tpmSanityCheck(); check != nil {
		...
		errorAndTime.Error = check.err.Error()
		errorAndTime.ErrorTime = time.Now()
		errorAndTime.ErrorSeverity = types.ErrorSeverityWarning
		errorAndTime.ErrorRetryCondition = getTpmSanityStatus(check.tpmErrorType)
...
func tpmSanityCheck() *tpmSanityCheckError {
	...
	// encrypt/decrypt the vualt key and send/recived it from controler.
	...
	// TODO : artifical error injection for testing
	err = fmt.Errorf("artificial error")

	// TODO : this goes to eve-api
	TPM_BACKUP_KEY_ENCRYPTION_WARNING := 5

	if err != nil {
		return &tpmSanityCheckError{
			fmt.Errorf("failed to encrypt key using TPM: %w", err),
			TPM_BACKUP_KEY_ENCRYPTION_WARNING,
		}
	}
...
func getTpmSanityStatus(status int) string {
	switch status {
	case 5: // TODO : add this to eve-api
