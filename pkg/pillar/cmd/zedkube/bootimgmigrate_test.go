...
// kvClientAvailable returns a mock whose KubeVirt.Get reports Available=True.
func kvClientAvailable(ctrl *gomock.Controller) kubecli.KubevirtClient {
	mc := kubecli.NewMockKubevirtClient(ctrl)
	mkv := kubecli.NewMockKubeVirtInterface(ctrl)
	mc.EXPECT().KubeVirt("kubevirt").Return(mkv).AnyTimes()
	mkv.EXPECT().Get(gomock.Any(), "kubevirt", metav1.GetOptions{}).Return(&virtv1.KubeVirt{
...
func TestBootImgMigratorStep_WaitReadyBothReady_ToMigrate(t *testing.T) {
	ctrl := gomock.NewController(t)
	// Each step() call advances exactly one state. When both conditions are met
	// in WaitReady, the state transitions to Migrate on that tick.
	// The Migrate → Done transition happens on the following tick (tested separately).
	m := bootImgMigrator{imageReady: true}
	m.step(testNode, testNS, kvClientAvailable(ctrl))
