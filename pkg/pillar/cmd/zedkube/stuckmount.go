...
// where cluster-init.sh still supervises k3s have no socket; there the only lever
// is to terminate the process and let the shell supervisor relaunch it. Detecting
// per attempt rather than once at start-up keeps a single pillar binary correct on
// both, including across an upgrade that swaps the supervisor underneath it.
