...
// Regex to match ANSI escape sequences for stripping from log lines of color codes
...
func (z *zedkube) collectAppLogs() {
	...
	// Cache all pods once and index virt-launcher pods for VMI apps.
	...
	for _, item := range items {
		...
		// First try the normal app pods (native containers) labeled with app=<kubeName> by ReplicaSet
		...
		if err != nil {
			logrus.Errorf("collectAppLogs: can't list pods by label %v", err)
			continue
		}
		found := false
		...
		for scanner.Scan() {
			...
			// Strip ANSI escape sequences (color codes) from the log line for cleaner output
			cleanLine := ansiRe.ReplaceAllString(logLine, "")
