...
// versioned tag was deleted by cleanup_old_external_boot_images.  That case
// self-heals: checkStuckPendingVMI (pendingvmi.go) force-deletes the stuck
// (Pending) VMI, and the ReplicaSet recreates it from the template — which this
// machine will have patched to :latest by then.  To keep that window small,
// step() collapses WaitReady → Migrate → Done into a single tick once both
// readiness conditions are met, rather than spending a tick per transition.
