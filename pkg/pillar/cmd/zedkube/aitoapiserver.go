...
func checkDelIoAdpaterEthernet(ctx *zedkubeContext, aiConfig *types.AppInstanceConfig) {
	...
	for _, io := range ioAdapter {
		if io.Type == types.IoNetEth {
			...
			log.Functionf("checkDelIoAdpaterEthernet: delete existing nad %v", nadname)
		}
	}
}
