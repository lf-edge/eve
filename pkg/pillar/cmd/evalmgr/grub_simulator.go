...
func SimulateGrubBoot(gpt GptAttributeAccess, log *base.LogObject) (string, error) {
	...
	for _, label := range validLabels {
		attr, err := gpt.GetPartitionAttributes(log, label)
		if err != nil {
			if log != nil {
	...
	attr, err := gpt.GetPartitionAttributes(log, partitionFound)
	if err != nil {
	...
	if triesLeft > 0 {
		...
		err = gpt.SetPartitionAttributes(log, partitionFound, attr)
		if err != nil {
