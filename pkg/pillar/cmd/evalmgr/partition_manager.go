...
func (pm *PartitionManager) MarkGood(log *base.LogObject, partition string) error {
	// Read current attributes
	attr, err := pm.store.GetPartitionAttributes(log, partition)
	if err != nil {
	...
	err = pm.store.SetPartitionAttributes(log, partition, 0x102)
	if err != nil {
...
func (pm *PartitionManager) MarkBad(log *base.LogObject, partition string) error {
	// Set to bad state: all zeros (0x000)
	err := pm.store.SetPartitionAttributes(log, partition, 0x000)
	if err != nil {
...
func (pm *PartitionManager) SetBest(log *base.LogObject, partition string) error {
	...
	err = pm.store.SetPartitionAttributes(log, partition, 0x103)
	if err != nil {
...
func (pm *PartitionManager) FindFailedPartitions(log *base.LogObject) ([]string, error) {
	var failed []string
