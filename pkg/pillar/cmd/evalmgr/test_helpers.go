...
func newTestEvalContext(ps *pubsub.PubSub, testCtx *TestContext, partitionMgr *PartitionManager, memFs afero.Fs) *evalMgrContext {
	return &evalMgrContext{
		...
		// Production: stabilityPeriod=5min, statusUpdate=30s, rebootTick=1s, stillRunning=25s
		// Test: 100x faster but same ratios for realistic behavior
		stabilityPeriod:      3000 * time.Millisecond, // 3 seconds (was 5 minutes = 300,000ms)
...
// gen_script creates a mock executable script that returns "executed: <script_name>"
func gen_script(path string, scriptName string) error {
...
func setupMockCommands(fs afero.Fs) (string, error) {
	...
	if err := gen_script(iommuScript, "iommu-groups.sh"); err != nil {
		os.RemoveAll(tmpDir)
		return "", err
	}

	specScript := filepath.Join(tmpDir, "spec.sh")
	if err := gen_script(specScript, "spec.sh"); err != nil {
		os.RemoveAll(tmpDir)
		return "", err
	}
...
func (tc *TestContext) SetPartitionState(partition string, state string) {
	...
	err := tc.MockPartitionManager.SetPartitionAttributes(nil, partition, attr)
	if err != nil {
