...
func getPartitionAttributes(log *base.LogObject, slot string) (uint16, error) {
	// Get parent disk
...
func setPartitionAttributes(log *base.LogObject, slot string, attr uint16) error {
	// Get parent disk
	rootDisk, err := getRootDisk()
	if err != nil {
...
func (c *CgptAccess) GetPartitionAttributes(log *base.LogObject, partition string) (uint16, error) {
	return getPartitionAttributes(log, partition)
}

// SetPartitionAttributes writes the raw GPT attribute value for a partition
// Uses cgpt command to write to actual disk
func (c *CgptAccess) SetPartitionAttributes(log *base.LogObject, partition string, attr uint16) error {
	return setPartitionAttributes(log, partition, attr)
}
