...
func (ctx *evalMgrContext) startStabilityTimer() {
	// Use configured stability period or default
	if ctx.stabilityPeriod == 0 {
		ctx.stabilityPeriod = DefaultStabilityPeriod
	}
...
func (ctx *evalMgrContext) handleStabilityTimeout() {
	...
	if err := ctx.partitionMgr.MarkGood(log, string(ctx.currentSlot)); err != nil {
...
func (ctx *evalMgrContext) finalizeEvaluation(state *types.EvalPersist) {
	...
	if err := ctx.partitionMgr.SetBest(log, string(bestSlot)); err != nil {
