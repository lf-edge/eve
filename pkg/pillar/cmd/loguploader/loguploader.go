...
func sendCtxInit(ps *pubsub.PubSub, ctx *loguploaderContext) {
	...
	log.Functionf("sendCtxInit: Get Device Serial %s, Soft Serial %s", cloudClient.DevSerial,
...
func handleDNSImp(ctxArg interface{}, key string, statusArg interface{}) {
	...
	*deviceNetworkStatus = status
	...
	if ctx.cloudClient != nil && ctx.cloudClient.UsingV2API() {
...
func sendToCloud(ctx *loguploaderContext, data []byte, iter int, fName string, fTime int, isApp bool) (bool, error) {
	...
	if isApp {
		...
		logsURL = cloudconn.URLPathString(ctx.serverNameAndPort, ctx.cloudClient.UsingV2API(),
	...
	ctxWork, cancel := ctx.cloudClient.GetContextForAllIntfFunctions()
	defer cancel()
	...
	rv, err := ctx.cloudClient.SendOnAllIntf(ctxWork, logsURL, buf, cloudconn.RequestOptions{
