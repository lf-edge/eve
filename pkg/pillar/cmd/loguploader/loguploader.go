...
const (
	...
	backoffMaxTimeout      = 3600 // backoff timeout in one hour
...
func Run(ps *pubsub.PubSub, loggerArg *logrus.Logger, logArg *base.LogObject) int {
	...
	loguploaderCtx.backoffExprTimer = time.NewTimer(backoffMaxTimeout * time.Second)
		...
		case <-loguploaderCtx.scheduleTimer.C:
			...
			if backoffMode {
				...
				if numLeftFiles < 5 {
					...
					interval = 15
				} else if numLeftFiles >= 200 && numLeftFiles < 1000 || prevBackoff < 7200 {
					// if recently had received code 429, keep minimum interval as 8 seconds
					interval = 8
				} else {
					interval = 3
				}
		...
		if resp.StatusCode == http.StatusOK {
			...
			if resp.StatusCode == http.StatusServiceUnavailable { // status code 503
				...
				if resp.StatusCode == http.StatusTooManyRequests {
					...
					ctx.backoffExprTimer = time.NewTimer(backoffMaxTimeout * time.Second)
					// double the upload interval up to a max
