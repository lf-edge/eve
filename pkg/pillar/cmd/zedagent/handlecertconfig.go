...
func verifySigningCertNewest(ctx *zedagentContext, certByte []byte) error {
	...
	err = cloudClient.LoadSavedServerSigningCert()
	if err != nil {
	...
	oldCert := cloudClient.GetServerSigningCert()
...
func requestCertsByURL(ctx *zedagentContext, certURL string, desc string,
	...
	rv, err := cloudClient.SendOnAllIntf(ctxWork, certURL, nil, cloudconn.RequestOptions{
	...
	if len(rv.RespContents) > 0 {
		err = cloudClient.RemoveAndVerifyAuthContainer(&rv, true)
		if err != nil {
	...
	signingCertBytes, ret := cloudClient.VerifyProtoSigningCertChain(rv.RespContents)
	if ret != nil {
	...
	if err := cloudClient.SaveServerSigningCert(signingCertBytes); err != nil {
...
func sendAttestReqProtobuf(
	...
	attestURL := cloudconn.URLPathString(serverNameAndPort, cloudClient.UsingV2API(),
		devUUID, "attest")
	...
	ctx.deferredEventQueue.SetDeferred(deferKey, buf, attestURL,
		attestReq.ReqType, cloudconn.DeferredItemOpts{
			BailOnHTTPErr:  false,
			WithNetTracing: false,
			IgnoreErr:      true,
		})
