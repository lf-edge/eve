...
func controllerCertsTask(ctx *zedagentContext, triggerCerts <-chan struct{}) {
	...
	certInterval := ctx.globalConfig.GlobalValueDurationSeconds(types.CertInterval)
	if retry {
	...
	for {
		...
		if retry && success {
			...
			updateTaskTimer(ctx.globalConfig.GlobalValueDurationSeconds(types.CertInterval),
