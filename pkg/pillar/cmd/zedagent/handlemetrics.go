...
func getAppIP(ctx *zedagentContext, aiStatus *types.AppInstanceStatus,
	...
	for _, ulStatus := range aiStatus.UnderlayNetworks {
		...
		log.Tracef("getAppIP(%s, %s) found underlay v4: %s, v6: %s assigned %v mac %s",
			aiStatus.Key(), vifname, ulStatus.AllocatedIPv4Addr,
			ulStatus.AllocatedIPv6List, ulStatus.Assigned, ulStatus.Mac)
		return ulStatus.AllocatedIPv4Addr, ulStatus.AllocatedIPv6List, ulStatus.Assigned, ulStatus.Mac, ulStatus.IPAddrMisMatch
	}
