...
func SendProtobuf(url string, buf *bytes.Buffer, iteration int) error {
	...
	defer cancel()
	rv, err := cloudClient.SendOnAllIntf(ctxWork, url, buf, cloudconn.RequestOptions{
...
func sendMetricsProtobufByURL(ctx *getconfigContext, metricsURL string,
	...
	ctxWork, cancel := cloudClient.GetContextForAllIntfFunctions()
	defer cancel()
	rv, err := cloudClient.SendOnAllIntf(ctxWork, metricsURL, buf,
...
func sendHardwareHealthProtobufByURL(ctx *getconfigContext, hardwareHealthURL string,
	...
	rv, err := cloudClient.SendOnAllIntf(ctxWork, hardwareHealthURL, buf,
		cloudconn.RequestOptions{
			WithNetTracing: false,
			BailOnHTTPErr:  false,
			Iteration:      iteration,
