...
func initializeLocalAppBootInfo(ctx *getconfigContext) {
	...
	if !loadSavedAppBootConfig(ctx) {
		...
		saveAppBootConfig(ctx, &profile.AppBootConfigList{AppConfigs: nil})
	}
}
...
func localAppBootInfoPOSTTask(ctx *getconfigContext) {
	...
	for _, bootConfig := range ctx.sideController.currentAppBootConfigs {
		...
		applyAppBootConfig(ctx, bootConfig.AppUUID, bootConfig.BootOrder)
		ctx.sideController.Lock()
	}
	ctx.sideController.Unlock()
...
func processReceivedAppBootConfig(ctx *getconfigContext, config *profile.AppBootConfigList) {
	...
	for _, protoConfig := range config.AppConfigs {
		...
		ctx.sideController.Unlock()
		applyAppBootConfig(ctx, appUUID, bootOrder)
		ctx.sideController.Lock()
		configChanged = true
	}
	...
	for uuidStr, bootConfig := range ctx.sideController.currentAppBootConfigs {
		if !seenApps[uuidStr] {
			...
			ctx.sideController.Unlock()
			applyAppBootConfig(ctx, bootConfig.AppUUID, zevecommon.BootOrder_BOOT_ORDER_UNSPECIFIED)
			ctx.sideController.Lock()
			configChanged = true
		}
	}
...
func applyAppBootConfig(ctx *getconfigContext, appUUID uuid.UUID, bootOrder zevecommon.BootOrder) {
...
func applyDevicePropertyBootOrder(ctx *getconfigContext, bootOrderStr string) (changed bool) {
	log.Noticef("applyDevicePropertyBootOrder: bootOrder=%q", bootOrderStr)
...
func saveAppBootConfig(ctx *getconfigContext, config *profile.AppBootConfigList) {
...
func getAppBootConfig(ctx *getconfigContext, appUUID uuid.UUID) *types.AppBootConfig {
	ctx.sideController.Lock()
	defer ctx.sideController.Unlock()
	if bootConfig, exists := ctx.sideController.currentAppBootConfigs[appUUID.String()]; exists {
		return &bootConfig
	}
	return nil
}
