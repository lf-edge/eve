...
type zedagentContext struct {
	...
	// this queue.
	deferredEventQueue *cloudconn.DeferredQueue
	...
	deferredPeriodicQueue *cloudconn.DeferredQueue
	// All periodic LOC HTTP requests are added to this queue,
	// sending errors of which can be ignored. This means even the
	// request has failed, it will be removed from the queue, so there
	// is no need to `kick` this queue once connectivity has restored.
	deferredLOCPeriodicQueue *cloudconn.DeferredQueue
...
var cloudClient *cloudconn.Client
...
func queueInfoToDest(ctx *zedagentContext, dest destinationBitset,
	...
	if dest&LOCDest != 0 && locConfig != nil {
		url := cloudconn.URLPathString(locConfig.LocURL, cloudClient.UsingV2API(),
			devUUID, "info")
...
func Run(ps *pubsub.PubSub, loggerArg *logrus.Logger, logArg *base.LogObject, arguments []string, baseDir string) int {
	...
	cloudClient = initZedcloudContext(getconfigCtx,
	...
	zedagentCtx.deferredEventQueue = cloudconn.CreateDeferredQueue(log, cloudClient,
		zedagentCtx.ps, agentName, "DeferredEvent",
		warningTime, errorTime,
		getDeferredSentHandlerFunction(zedagentCtx),
		getDeferredPriorityFunctions()...)
	zedagentCtx.deferredPeriodicQueue = cloudconn.CreateDeferredQueue(log, cloudClient,
		zedagentCtx.ps, agentName, "DeferredPeriodic",
		warningTime, errorTime, nil)
	zedagentCtx.deferredLOCPeriodicQueue = cloudconn.CreateDeferredQueue(log, cloudClient,
...
func (zedagentCtx *zedagentContext) init() {
	zedagentCtx.agentMetrics = cloudconn.NewAgentMetrics()
...
func handleDNSImpl(ctxArg interface{}, key string,
	...
	*deviceNetworkStatus = status
	...
	if cloudClient.UsingV2API() {
		cloudClient.UpdateTLSProxyCerts()
	}
...
func getDeferredSentHandlerFunction(ctx *zedagentContext) cloudconn.SentHandlerFunction {
...
func getDeferredPriorityFunctions() []cloudconn.TypePriorityCheckFunction {
	var functions []cloudconn.TypePriorityCheckFunction
