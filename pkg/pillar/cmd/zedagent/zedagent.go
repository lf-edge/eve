...
func Run(ps *pubsub.PubSub) {
	...
	for {
		...
		case change := <-cipherCtx.subEveNodeCertStatus.MsgChan():
			cipherCtx.subEveNodeCertStatus.ProcessChange(change)

		case change := <-cipherCtx.subControllerCertStatus.MsgChan():
			cipherCtx.subControllerCertStatus.ProcessChange(change)

		case change := <-cipherCtx.subCipherContextStatus.MsgChan():
			cipherCtx.subCipherContextStatus.ProcessChange(change)

		case change := <-cipherCtx.subNimCipherBlockStatus.MsgChan():
			cipherCtx.subNimCipherBlockStatus.ProcessChange(change)

		case change := <-cipherCtx.subDomainMgrCipherBlockStatus.MsgChan():
			cipherCtx.subDomainMgrCipherBlockStatus.ProcessChange(change)

		case change := <-cipherCtx.subDownloaderCipherBlockStatus.MsgChan():
			cipherCtx.subDownloaderCipherBlockStatus.ProcessChange(change)
