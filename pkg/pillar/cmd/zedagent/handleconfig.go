...
func getLatestConfig(url string, iteration int,
	...
	if err != nil {
		...
		if rtf == types.SenderStatusRemTempFail {
			...
			// trigger event to acquire new certs from cloud
			triggerControllerCertsEvent(getconfigCtx.zedagentCtx)
		} else {
...
func writeSentCertsProtoMessage(contents []byte) {
	writeProtoMessage("lastcert", contents)
}
