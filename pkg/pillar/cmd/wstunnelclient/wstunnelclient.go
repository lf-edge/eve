...
type wstunnelclientContext struct {
	...
	wstunnelclient       *cloudconn.WSTunnelClient
...
func scanAIConfigs(ctx *wstunnelclientContext) {
	...
	for _, port := range deviceNetworkStatus.Ports {
		...
		wstunnelclient := cloudconn.InitializeTunnelClient(log, ctx.serverNameAndPort, "localhost:4822")
		...
		for retryCount := 0; retryCount < addrCount; retryCount++ {
			...
			proxyURL, _ := cloudconn.LookupProxy(log, deviceNetworkStatus,
				ifname, destURL)
