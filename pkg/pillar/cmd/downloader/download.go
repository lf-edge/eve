...
func download(ctx *downloaderContext, trType zedUpload.SyncTransportType,
	...
	proxyLookupURL := cloudconn.IntfLookupProxyCfg(log, &ctx.deviceNetworkStatus, ifname, downloadURL, trType)
	proxyURL, err := cloudconn.LookupProxy(log, &ctx.deviceNetworkStatus, ifname, proxyLookupURL)
	if err != nil {
