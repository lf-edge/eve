...
func handleAppImgResolveModify(ctxArg interface{}, key string,
	...
	config := configArg.(types.AppImgResolveConfig)
...
func publishResolveStatus(ctx *downloaderContext,
	status *types.AppImgResolveStatus) {
...
func resolveTagsToHash(ctx *downloaderContext, rc types.AppImgResolveConfig) {
	var (
		err                           error
	...
	if rs == nil {
		rs = &types.AppImgResolveStatus{
			AppID:        rc.AppID,
			ImageID:      rc.ImageID,
			Name:         rc.Name,
			PurgeCounter: rc.PurgeCounter,
		}
	}
	...
	dsCtx, err := constructDatastoreContext(ctx, rc.Name, rc.NameIsURL, *dst)
	if err != nil {
