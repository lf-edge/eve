...
func (c *Client) LoadSavedServerSigningCert(useBackup bool) error {
	...
	// If we have an ancient config checkpoint and associated controllercerts
	// then the certs might have expired and the verification will fail.
	// Thus there is an implicit assumption that when the controller
	// signining cert is rotated (about once a year) that results in an updated
	// signed configuration being received by the device (and checkpointed)
	// before that cert expires (a year later). If not, a disconnected
	// device will not be able to use its checkpointed config.
