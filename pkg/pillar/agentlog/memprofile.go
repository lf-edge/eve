...
func MemoryPSICollector(log *base.LogObject, collectorCtx context.Context) error {
	ticker := time.NewTicker(PSITimeIntervalSec * time.Second)
	defer ticker.Stop()

	// Open the statistics file. Truncate it if it exists.
	statFile, err := os.OpenFile(types.MemoryMonitorPSILogFile, os.O_CREATE|os.O_RDWR|os.O_SYNC, 0644)
	if err != nil {
