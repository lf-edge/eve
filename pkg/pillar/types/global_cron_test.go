...
func TestCronFieldMatch(t *testing.T) {
	tests := []struct {
		...
		// Wildcard.
		{"wildcard matches any", "*", 42, false, true},
		{"wildcard matches zero", "*", 0, false, true},

		// Single numeric.
		{"single value match", "5", 5, false, true},
		{"single value no match", "5", 6, false, false},
		{"zero matches zero", "0", 0, false, true},

		// Comma list.
		{"list: first element matches", "1,3,5", 1, false, true},
		{"list: middle element matches", "1,3,5", 3, false, true},
		{"list: last element matches", "1,3,5", 5, false, true},
		{"list: no element matches", "1,3,5", 2, false, false},

		// Weekday Saturday+Sunday (default schedule).
		{"6,0 Saturday matches", "6,0", 6, true, true},
		{"6,0 Sunday matches", "6,0", 0, true, true},
		{"6,0 Monday no match", "6,0", 1, true, false},

		// Weekday 7 as Sunday in comma list.
		{"6,7 Saturday matches", "6,7", 6, true, true},
		{"6,7 Sunday (0) matches via 7", "6,7", 0, true, true},
		{"6,7 Monday no match", "6,7", 1, true, false},

		// Range.
		{"range 1-5 lo matches", "1-5", 1, false, true},
		{"range 1-5 mid matches", "1-5", 3, false, true},
		{"range 1-5 hi matches", "1-5", 5, false, true},
		{"range 1-5 below lo no match", "1-5", 0, false, false},
		{"range 1-5 above hi no match", "1-5", 6, false, false},

		// Step.
		{"*/5 matches 0", "*/5", 0, false, true},
		{"*/5 matches 5", "*/5", 5, false, true},
		{"*/5 matches 55", "*/5", 55, false, true},
		{"*/5 no match 3", "*/5", 3, false, false},
	}

	for _, tc := range tests {
		t.Run(tc.name, func(t *testing.T) {
			got := cronFieldMatch(tc.field, tc.val, tc.isWeekday)
			if got != tc.want {
				t.Errorf("cronFieldMatch(%q, %d, %v) = %v, want %v",
					tc.field, tc.val, tc.isWeekday, got, tc.want)
			}
		})
	}
}
...
func TestCronAtomMatch(t *testing.T) {
	tests := []struct {
		...
		// Wildcard.
		{"* matches any non-weekday", "*", 30, false, true},
		{"* matches weekday", "*", 3, true, true},

		// Single value.
		{"exact match", "2", 2, false, true},
		{"exact no match", "2", 3, false, false},
		{"zero exact match", "0", 0, false, true},

		// Weekday Sunday duality.
		{"weekday 0 matches Sunday val 0", "0", 0, true, true},
		{"weekday 7 matches Sunday val 0", "7", 0, true, true},
		{"weekday 7 no match Saturday val 6", "7", 6, true, false},
		{"weekday 6 matches Saturday", "6", 6, true, true},
		{"non-weekday 7 matches val 7", "7", 7, false, true},

		// Range.
		{"range lo boundary", "1-5", 1, false, true},
		{"range mid", "1-5", 3, false, true},
		{"range hi boundary", "1-5", 5, false, true},
		{"range below lo", "1-5", 0, false, false},
		{"range above hi", "1-5", 6, false, false},

		// Range including weekday 7 (5-7 covers Fri, Sat, Sun).
		{"weekday range 5-7 Friday matches", "5-7", 5, true, true},
		{"weekday range 5-7 Saturday matches", "5-7", 6, true, true},
		{"weekday range 5-7 Sunday via 7 matches", "5-7", 0, true, true},
		{"weekday range 5-7 Monday no match", "5-7", 1, true, false},

		// Step: */n.
		{"*/5 minute 0", "*/5", 0, false, true},
		{"*/5 minute 5", "*/5", 5, false, true},
		{"*/5 minute 10", "*/5", 10, false, true},
		{"*/5 minute 55", "*/5", 55, false, true},
		{"*/5 minute 3 no match", "*/5", 3, false, false},
		{"*/5 minute 57 no match", "*/5", 57, false, false},
		{"*/15 minute 0", "*/15", 0, false, true},
		{"*/15 minute 15", "*/15", 15, false, true},
		{"*/15 minute 30", "*/15", 30, false, true},
		{"*/15 minute 45", "*/15", 45, false, true},
		{"*/15 minute 1 no match", "*/15", 1, false, false},

		// Step: range/n.
		{"1-6/2 matches 1", "1-6/2", 1, false, true},
		{"1-6/2 matches 3", "1-6/2", 3, false, true},
		{"1-6/2 matches 5", "1-6/2", 5, false, true},
		{"1-6/2 no match 2", "1-6/2", 2, false, false},
		{"1-6/2 no match 4", "1-6/2", 4, false, false},
		{"1-6/2 no match 6 (beyond last step)", "1-6/2", 6, false, false},

		// Invalid inputs — should return false, not panic.
		{"non-numeric atom", "abc", 0, false, false},
		{"step zero invalid", "*/0", 0, false, false},
		{"invalid range hi", "1-abc", 1, false, false},
		{"invalid range lo", "abc-5", 1, false, false},
		{"invalid step suffix", "1-5/x", 1, false, false},
	}

	for _, tc := range tests {
		t.Run(tc.name, func(t *testing.T) {
			got := cronAtomMatch(tc.atom, tc.val, tc.isWeekday)
			if got != tc.want {
				t.Errorf("cronAtomMatch(%q, %d, %v) = %v, want %v",
					tc.atom, tc.val, tc.isWeekday, got, tc.want)
			}
		})
	}
}
