...
func UpdateTLSProxyCerts(ctx *ZedCloudContext) bool {
	...
	if tlsCfg == nil || devNS == nil {
		ctx.log.Errorln("UpdateTLSProxyCerts: tlsconfig or dev NS missing")
		return false
	}
	...
	if len(ctx.PrevCertPEM) > 0 {
		...
		ctx.log.Functionf("UpdateTLSProxyCerts: rebuild root CA\n")
	} else {
	...
	ctx.log.Functionf("UpdateTLSProxyCerts: root CA updated")
