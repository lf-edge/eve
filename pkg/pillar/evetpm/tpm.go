...
var (
	...
	TpmMeasurefsEventLog = types.PersistStatusDir + "/measurefs_tpm_event_log"
...
func copyMeasurementLog(dstPath string) error {
	var appendErr error
	tpmEventLog, err := os.ReadFile(TpmMeasurementLogFile)
	if err != nil {
		return fmt.Errorf("failed to read TPM measurements log file: %w", err)
	}

	measurefsEventLog, err := os.ReadFile(TpmMeasurefsEventLog)
	if err == nil {
