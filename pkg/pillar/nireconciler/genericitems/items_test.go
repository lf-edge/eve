...
func TestDnsmasqName(t *testing.T) {
	d := Dnsmasq{ListenIf: NetworkIf{IfName: "br0"}}
	if got := d.Name(); got != "br0" {
		t.Errorf("got %q, want %q", got, "br0")
	}
}

func TestDnsmasqLabel(t *testing.T) {
	d := Dnsmasq{ListenIf: NetworkIf{IfName: "br0"}}
	if got := d.Label(); got != "dnsmasq for br0" {
		t.Errorf("got %q", got)
	}
}

func TestDnsmasqType(t *testing.T) {
	d := Dnsmasq{}
	if got := d.Type(); got != DnsmasqTypename {
		t.Errorf("got %q, want %q", got, DnsmasqTypename)
	}
}

func TestDnsmasqExternal(t *testing.T) {
	d := Dnsmasq{}
	if d.External() {
...
func TestDnsmasqString(t *testing.T) {
	d := Dnsmasq{ListenIf: NetworkIf{IfName: "br0"}}
	if s := d.String(); s == "" {
...
func TestHTTPServerName(t *testing.T) {
	s := HTTPServer{ListenIf: NetworkIf{IfName: "br0"}, Port: 8080}
	if got := s.Name(); got != "br0:8080" {
		t.Errorf("got %q, want %q", got, "br0:8080")
	}
}

func TestHTTPServerLabel(t *testing.T) {
	s := HTTPServer{ListenIf: NetworkIf{IfName: "br0"}, Port: 8080}
	if got := s.Label(); got != "HTTP server for br0:8080" {
		t.Errorf("got %q", got)
	}
}
...
func TestHTTPServerType(t *testing.T) {
	s := HTTPServer{}
	if got := s.Type(); got != HTTPServerTypename {
		t.Errorf("got %q, want %q", got, HTTPServerTypename)
	}
}

func TestHTTPServerExternal(t *testing.T) {
	s := HTTPServer{}
	if s.External() {
		t.Error("expected External() = false")
	}
...
func TestIPReserveName(t *testing.T) {
	ip := &net.IPNet{IP: net.IP{10, 0, 0, 1}, Mask: net.CIDRMask(24, 32)}
	r := IPReserve{AddrWithMask: ip}
	if got := r.Name(); got != "10.0.0.1" {
		t.Errorf("got %q, want %q", got, "10.0.0.1")
	}
}

func TestIPReserveLabel(t *testing.T) {
	ip := &net.IPNet{IP: net.IP{10, 0, 0, 1}, Mask: net.CIDRMask(24, 32)}
	r := IPReserve{AddrWithMask: ip}
	if got := r.Label(); got != "10.0.0.1/24" {
		t.Errorf("got %q, want %q", got, "10.0.0.1/24")
	}
}

func TestIPReserveType(t *testing.T) {
	r := IPReserve{AddrWithMask: &net.IPNet{IP: net.IP{10, 0, 0, 1}, Mask: net.CIDRMask(24, 32)}}
	if got := r.Type(); got != IPReserveTypename {
		t.Errorf("got %q, want %q", got, IPReserveTypename)
	}
}

func TestIPReserveExternal(t *testing.T) {
	r := IPReserve{AddrWithMask: &net.IPNet{IP: net.IP{10, 0, 0, 1}, Mask: net.CIDRMask(24, 32)}}
	if r.External() {
		t.Error("expected External() = false")
	}
...
func TestIPReserveString(t *testing.T) {
	r := IPReserve{
		AddrWithMask: &net.IPNet{IP: net.IP{10, 0, 0, 1}, Mask: net.CIDRMask(24, 32)},
		NetIf:        NetworkIf{IfName: "br0"},
	}
	if s := r.String(); s == "" {
		t.Error("expected non-empty string")
	}
...
func TestPortLabel(t *testing.T) {
	p := Port{IfName: "eth0", LogicalLabel: "uplink"}
	if got := p.Label(); got != "uplink (eth0)" {
		t.Errorf("got %q", got)
	}
}
...
func TestPortType(t *testing.T) {
	p := Port{}
	if got := p.Type(); got != PortTypename {
		t.Errorf("got %q, want %q", got, PortTypename)
	}
}

func TestPortExternal(t *testing.T) {
	p := Port{}
	if !p.External() {
...
func TestRadvdName(t *testing.T) {
	r := Radvd{ListenIf: NetworkIf{IfName: "br0"}}
	if got := r.Name(); got != "br0" {
		t.Errorf("got %q, want %q", got, "br0")
	}
}

func TestRadvdLabel(t *testing.T) {
	r := Radvd{ListenIf: NetworkIf{IfName: "br0"}}
	if got := r.Label(); got != "radvd for br0" {
		t.Errorf("got %q", got)
	}
}

func TestRadvdType(t *testing.T) {
	r := Radvd{}
	if got := r.Type(); got != RadvdTypename {
		t.Errorf("got %q, want %q", got, RadvdTypename)
	}
}

func TestRadvdExternal(t *testing.T) {
	r := Radvd{}
	if r.External() {
		t.Error("expected External() = false")
	}
...
func TestRadvdString(t *testing.T) {
	r := Radvd{ListenIf: NetworkIf{IfName: "br0"}, MTU: 1500}
	if s := r.String(); s == "" {
		t.Error("expected non-empty string")
	}
}
