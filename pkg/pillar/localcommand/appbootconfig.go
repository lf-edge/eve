...
//   - 204 (No Content): No changes since last fetch, returns nil (cache preserved)
//   - 404 (Not Found): LPS has no config for this device, returns empty config
//     to clear all LPS-set boot orders. Enables throttled polling (5 min) to reduce load.
	...
	for intf, srvAddrs := range lc.lpsAddresses.addrsByIface {
		for _, srvAddr := range srvAddrs {
			...
			case http.StatusNotFound:
				...
				// Throttle polling to reduce load on LPS.
	...
	for appUUID := range lc.currentAppBootConfigs {
		if !seenApps[appUUID] {
			lc.Log.Noticef("%s: Resetting BootOrder for app no longer in LPS config: %s",
				logPrefix, appUUID.String())
			lc.ConfigAgent.ApplyAppBootConfig(appUUID, zcommon.BootOrder_BOOT_ORDER_UNSPECIFIED)
