...
`HandlerOptions{TpmKeyOnlyMode}` is set at startup by `vaultmgr` from
the persistent `VaultConfig` it owns, or to TPM-key-only when there is
no `VaultConfig` to read; the handler hands the flag down to
`stageKey`. It names the derivation to try first, not a fact: a failed
unlock is retried with the other derivation (`resolveKeyMode`,
`keymode.go`) and whichever one opened the vault is stored back into
the options. `vaultmgr` reads that through `GetHandlerOptions` and
persists it whenever it disagrees with what is on disk.

The retry is unconditional because a recorded mode can be wrong in two
ways: it may be a guess an earlier EVE wrote out before trying it, and
a vault recreate keys the replacement TPM-key-only whatever the old
vault used. Neither is distinguishable from a bad seal at the point of
failure -- the TPM unseal succeeds and only the filesystem's own key
check refuses -- and a key that does not match the vault changes
nothing on disk.
...
| Vault create (`SetupDefaultVault`) | `fscrypt encrypt /persist/vault --key=/TmpVaultDir2/protector.key --source=raw_key --name=TheVaultKey<basename> --user=root`, with the raw key staged on tmpfs at `/TmpVaultDir2/protector.key`. |
| Vault unlock (`UnlockDefaultVault`) | `fscrypt unlock /persist/vault --key=/TmpVaultDir2/protector.key --user=root`, then `keyctl link @u @s` to expose the policy key in the session keyring. |
...
| `tpmKeyOnlyMode=false` *and* `cloudKeyOnlyMode=false` | returns `mergeKeys(tpmKey, cloudKey)` = first 16 B of TPM key ‖ second 16 B of `cloudKey` | Only on devices first installed at 5.6.2 ≤ EVE < 7.10.0 that have since been upgraded. `TpmKeyOnly` is sticky in the persistent `VaultConfig` — once `false`, it stays `false`. Losing that file does not silently move a device between the two branches: a mode that does not unlock is retried against the other branch, and only the branch that worked is written back. |
