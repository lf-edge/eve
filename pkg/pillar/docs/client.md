...
* **`getUuid`** — call the controller's `/uuid` endpoint with the
  ...
  hardware-model override. `zedclient` exits once either operation
  reports success, but it publishes
  `OnboardingStatus{DeviceUUID,HardwareModel}` only when it has a
  device UUID — which only `getUuid` (or the cached-UUID fast path)
  provides; a `selfRegister`-only success sets no UUID and publishes
  nothing,
...
* `OnboardingStatus` with key `"global"` (persistent) — the single
  artifact every other agent waits for. Carries `DeviceUUID` and
...
* on-disk side effects
  ...
  * `/persist/checkpoint/controllercerts` — protobuf-encoded V2 cert
    chain (`ZControllerCert`; the leaf certs inside are DER), written
    via `persist.MaybeSaveControllerCerts` (only when
...
* `Run()` — the entire lifecycle. Builds the `clientContext`,
  ...
  publications, brings up the two subscriptions,
  constructs the `controllerconn.Client`, loads the
...
Run()
  ...
  └─ subscribe ConfigItemValueMap, DeviceNetworkStatus
  ...
  └─ event loop until done:
       ...
       ├ GlobalConfig change → process, no register
...
Before trusting any observed `zedclient` behavior on an existing
...
* To exercise the `selfRegister` failure paths, point `/config/server`
  ...
  controller honoring the eve-api `/register` spec — a controller
  that returns 401 instead leaves the `LedBlinkOnboardingFailureNotFound`
  branch dead.
* The `SenderStatusCertMiss` branch in `doGetUUID` is not reachable
  in `zedclient`'s lifetime from e2e: `tryRegister` fetches the cert
  chain (gated on `!gotServerCerts`) and returns early until it
  succeeds, so `fetchCertChain` has always refreshed the cached
  signing-cert hash before `doGetUUID` is ever reached, and failure
  paths from `/uuid` (404/401) return plain HTTP errors without an
