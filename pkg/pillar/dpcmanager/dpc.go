...
// higherPriority reports whether incomingDPC should be placed before (i.e.
// at a higher priority than) existingDPC in the DPC list. The check is
// intentionally asymmetric: it matters which DPC is arriving now
// (incomingDPC) versus which one is already in the list (existingDPC).
//
// A newly-submitted "manual" DPC always becomes the highest-priority entry
// immediately, regardless of existingDPC - there is at most one "manual" at
// a time, so no timestamp comparison is needed for it.
//
// A newly-arrived "zedagent" (controller) DPC always outranks an existing
// non-controller DPC, regardless of TimePriority - the controller's clock
// can't be reliably compared to the device's local/arbitrary one. Multiple
// "zedagent" DPCs can coexist though, so among those TimePriority still
// decides.
//
// Everything else falls back to plain TimePriority comparison, as before.
func higherPriority(incomingDPC, existingDPC types.DevicePortConfig) bool {
	if incomingDPC.Key == types.ManualDPCKey {
		return true
	}
	if incomingDPC.IsFromController() && !existingDPC.IsFromController() {
		return true
	}
	return incomingDPC.TimePriority.After(existingDPC.TimePriority)
}

// Insert in priority order (see higherPriority).
func (m *DpcManager) insertDPC(dpc types.DevicePortConfig) {
	...
	for _, port := range m.dpcList.PortConfigList {
		if !inserted && higherPriority(dpc, port) {
...
//   - The "manual" DPC (a singleton created via TUI), but only until some
//     higher-priority (more recent) DPC has provided working connectivity.
//     Once that happens, the manual DPC is no longer unconditionally kept by
//     this rule (though it may still be retained by another rule above,
//     e.g. being the currently used DPC or itself the most recent working one)
...
func (m *DpcManager) compressDPCL() {
	...
	for _, dpc := range m.dpcList.PortConfigList {
		if dpc.IsFromController() && !dpc.LastSucceeded.IsZero() {
	...
	for i, dpc := range m.dpcList.PortConfigList {
		...
		if dpc.IsFromController() && !alreadyRetainedKey {
		...
		if dpc.Key == types.ManualDPCKey && !retainedWorkingDPC {
			// Retain the "manual" DPC (there is always at most one) only as
			// long as no higher-priority DPC has ever provided working
			// connectivity. Once one has, the manual (TUI-submitted) config
			// is considered obsolete here.
			keep = true
		}
