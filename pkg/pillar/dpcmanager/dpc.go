...
func (m *DpcManager) compressDPCL() {
	...
	var hasLatestControllerDPC, haveWorkingDPC bool
	for i, dpc := range m.dpcList.PortConfigList {
		...
		if !haveWorkingDPC && dpc.WasDPCWorking() {
			// Do not remove the most recent DPC with working connectivity.
			keep = true
		}
		if !hasLatestControllerDPC && dpc.Key == "zedagent" {
