...
var testMatrix = map[string]compressDPCLTestEntry{
	...
	// Now, we compress the DPCL regardless, to for example prevent DPC from USB stick
	// from blocking compression.
	// However, we always keep the latest DPC from the controller, even if it
	// does not currently provide working connectivity or has been overridden.
	...
	// has been overridden by DPC from USB stick.
