...
func TestCreateDomConfigArm64BootOrderIgnored(t *testing.T) {
	...
	// ARM (virt machine) doesn't use OVMF, so fw_cfg boot order should NOT be present
	if bytes.Contains(result, []byte(`name = "opt/eve.bootorder"`)) {
		t.Error("unexpected opt/eve.bootorder fw_cfg on ARM - boot order should be ignored for virt machine")
	}
}
