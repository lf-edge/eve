...
[drive]
  ...
  format = "raw"
  unit = "1"
  file = "` + kvmStateDir + `{{.DisplayName}}/OVMF_VARS.fd"
...
func (ctx kvmContext) Setup(status types.DomainStatus, config types.DomainConfig, aa *types.AssignableAdapters, file *os.File) error {
	...
	source, err := os.Open("/usr/lib/xen/boot/OVMF_VARS.fd")
	if err != nil {
		return logError("Failed to open the source vars file: %v", err)
	}
	defer source.Close()

	destination, err := os.Create(kvmStateDir + domainName + "/OVMF_VARS.fd")
	if err != nil {
		return logError("Failed to open the destination vars file: %v", err)
	}
	defer destination.Close()

	_, err = io.Copy(destination, source)
	if err != nil {
		return logError("Failed to copy source to destination: %v", err)
	}
