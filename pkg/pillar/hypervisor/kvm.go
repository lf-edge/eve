...
type tQemuGlobalConfContext struct {
	...
	DumpGuestCore          bool   // true when debug.qemu.dump.guest.core is set; includes guest RAM in qemu core dumps if qemu crashes
...
const qemuTraceDir = "/persist/log/qemu-trace/"
...
func (ctx KvmContext) Setup(status types.DomainStatus, config types.DomainConfig,
	...
	if globalConfig != nil {
		...
		if traceEvents != "" {
			if traceArgs, err := setupQemuTrace(domainName, traceEvents); err != nil {
...
// setupQemuTrace materializes the comma-separated event list from
// `debug.qemu.trace.events` into a newline-separated file qemu can read
// via `-trace events=FILE`, and returns the qemu argv fragment that
// enables tracing into a per-domain simpletrace-format file under
// /persist/log/qemu-trace/.  Decode the resulting file with
// `scripts/simpletrace.py /usr/share/qemu-xen/trace-events-all <file>`.
func setupQemuTrace(domainName, eventsCSV string) ([]string, error) {
	...
	for _, ev := range strings.Split(eventsCSV, ",") {
	...
	if b.Len() == 0 {
		return nil, fmt.Errorf("no events in %q", eventsCSV)
	}
	if err := os.WriteFile(eventsFile, []byte(b.String()), 0644); err != nil {
		return nil, fmt.Errorf("write %s: %w", eventsFile, err)
	}
	traceFile := qemuTraceDir + domainName + ".trace"
...
func (ctx KvmContext) CreateDomConfig(domainName string,
	...
	var efiDebug, dumpGuestCore bool
	var gopRomFilename string
	if globalConfig != nil {
		efiDebug = globalConfig.GlobalValueBool(types.EnableEFIDebug)
		dumpGuestCore = globalConfig.GlobalValueBool(types.QemuDumpGuestCore)
