...
type tQemuDiskContext struct {
	...
	AioType                          string
	// UseIOThread attaches virtio-blk to the iothread0 object declared by the
	// global template. Both are emitted together or not at all; see
	// wantsBlockIOThread for when and why.
	UseIOThread bool
	types.DiskStatus
}
...
func (ctx KvmContext) GetCapabilities() (*types.Capabilities, error) {
	...
	ctx.capabilities = &types.Capabilities{
		...
		CPUPinning:               true,
		// pinDomainThreads binds each vCPU thread over QMP and the domain
		// config emits the matching -smp sockets/cores/threads.
		CPUTopologyPinning: true,
		UseVHost:           true,
	}
	return ctx.capabilities, nil
}
...
// legacyThreadContextArgs returns the qemu arguments for the legacy CPU pinning
// path, or nil when that path does not apply. CPU affinity cannot be expressed
// in the .ini file, hence the command line. The format is
// -object thread-context,id=tc1,cpu-affinity=0-1,cpu-affinity=6-7 and the
// thread-context object exists since qemu 7.2.
//
// Legacy pinning and topology pinning are two mechanisms for the same job and
// only one may be in force, which is what OrderedCPUs selects between.
// thread-context carries a single affinity mask, so it can only confine the
// domain's threads to status.CPUs as a set; it cannot say which host CPU a given
// guest vCPU gets, nor hold emulator threads and vCPU threads apart. Both of
// those are the whole point of the topology path, which instead pins each thread
// individually over QMP in Start() (see pinning.go). Emitting the mask as well
// would state a placement that contradicts the intended one: for a topology-
// pinned domain status.CPUs is the union of the vCPU cores, the SMT siblings
// parked idle next to them and -- under io_placement=housekeeping -- the shared
// housekeeping pool.
func legacyThreadContextArgs(config types.DomainConfig, status types.DomainStatus) []string {
	...
	return []string{"-object", threadContext}
}
...
func (ctx KvmContext) Setup(status types.DomainStatus, config types.DomainConfig,
	...
	args = append(args, legacyThreadContextArgs(config, status)...)
...
// wantsBlockIOThread reports whether this domain gets a dedicated iothread for
// its virtio-blk devices instead of running the block datapath on the QEMU main
// loop.
//
// Gated on EmulatorCPUs, i.e. on io_placement=housekeeping having actually been
// granted a housekeeping CPU set, because that is the only configuration in
// which the block datapath is meant to live somewhere other than where the
// vCPUs run: pinDomainThreads moves every non-vCPU thread onto that set, and
// giving virtio-blk a thread of its own is what keeps its work from being
// serialised behind the main loop's other duties there. Any other domain --
// unpinned, legacy-pinned, or topology-pinned with io_placement=dedicated --
// keeps the pre-existing single-threaded layout, since moving its block IO off
// the main loop would change the IO behaviour of every VM on every device
// without buying any vCPU isolation.
func wantsBlockIOThread(status types.DomainStatus) bool {
	return len(status.EmulatorCPUs) > 0
}
...
func (ctx KvmContext) CreateDomConfig(domainName string,
	...
	diskContext := tQemuDiskContext{
		...
		UseIOThread: wantsBlockIOThread(status),
	}
...
func (ctx KvmContext) Start(domainName string) error {
	...
	if err := ctx.pinDomainThreads(domainName, qmpFile, ordered, emulator); err != nil {
		return logError("failed to pin CPU threads for domain %s: %v", domainName, err)
	}
