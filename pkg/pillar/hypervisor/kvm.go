...
func (ctx kvmContext) CreateDomConfig(domainName string, config types.DomainConfig, diskStatusList []types.DiskStatus,
	...
	currentPCIType := "pcie"
	//we use PC machine type if we found IGD and use ovmf bootloader
	usePCMachineType = hasIGD && config.BootLoader == "/usr/lib/xen/boot/ovmf.bin"
	if usePCMachineType {
		currentPCIType = "pci"
	}
	...
	if len(pciAssignments) != 0 {
		...
		for _, pa := range pciAssignments {
			...
			if usePCMachineType && short == igdAddress {
