...
# The "virt" machine type (used on ARM) doesn't use OVMF, so we exclude it.
...
func (ctx KvmContext) CreateDomConfig(domainName string,
	...
	// Read boot order from VM config (controlled by LPS or Controller API)
	...
	if bootOrder == "" && globalConfig != nil {
		// Fall back to device-wide default from global config (string property)
		bootOrder = globalConfig.GlobalValueString(types.AppBootOrder)
	}
