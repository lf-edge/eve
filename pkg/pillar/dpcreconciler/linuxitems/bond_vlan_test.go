...
func TestBondName(t *testing.T) {
	t.Parallel()
	b := Bond{IfName: "bond0"}
	if got := b.Name(); got != "bond0" {
		t.Errorf("Name() = %q, want %q", got, "bond0")
	}
}

func TestBondLabel(t *testing.T) {
	t.Parallel()
	b := Bond{LogicalLabel: "uplink-bond", IfName: "bond0"}
	if got := b.Label(); !strings.Contains(got, "uplink-bond") {
		t.Errorf("Label() = %q, want it to contain logical label", got)
	}
}
...
func TestBondType(t *testing.T) {
	t.Parallel()
	b := Bond{}
	if got := b.Type(); got != genericitems.BondTypename {
		t.Errorf("Type() = %q, want %q", got, genericitems.BondTypename)
	}
}

func TestBondExternal(t *testing.T) {
	t.Parallel()
	b := Bond{}
	if b.External() {
...
func TestBondString(t *testing.T) {
	t.Parallel()
	b := Bond{IfName: "bond0", LogicalLabel: "uplink-bond"}
	s := b.String()
	if !strings.Contains(s, "bond0") {
		t.Errorf("String() missing IfName, got: %s", s)
	}
}
...
func TestBondDependencies(t *testing.T) {
	...
	// dep[0] is for "eth0", dep[1] for "eth1".
	// MustSatisfy: requires IOUsageBondAggrIf and MasterIfName == "bond0".
	goodPhysIf := PhysIf{
		PhysIfName:   "eth0",
	...
	if !deps[0].MustSatisfy(goodPhysIf) {
		t.Error("MustSatisfy(correct physif) = false, want true")
	}
	if deps[0].MustSatisfy(wrongUsage) {
		t.Error("MustSatisfy(wrong usage) = true, want false")
	}
	if deps[0].MustSatisfy(wrongMaster) {
...
func TestVlanName(t *testing.T) {
	t.Parallel()
	v := Vlan{IfName: "eth0.100"}
	if got := v.Name(); got != "eth0.100" {
		t.Errorf("Name() = %q, want %q", got, "eth0.100")
	}
}

func TestVlanLabel(t *testing.T) {
	t.Parallel()
	v := Vlan{LogicalLabel: "vlan-uplink"}
	if got := v.Label(); got != "vlan-uplink" {
		t.Errorf("Label() = %q, want %q", got, "vlan-uplink")
	}
}

func TestVlanType(t *testing.T) {
	t.Parallel()
	v := Vlan{}
	if got := v.Type(); got != genericitems.VlanTypename {
		t.Errorf("Type() = %q, want %q", got, genericitems.VlanTypename)
	}
}

func TestVlanExternal(t *testing.T) {
	t.Parallel()
	v := Vlan{}
	if v.External() {
		t.Error("External() = true, want false")
...
func TestVlanString(t *testing.T) {
	t.Parallel()
	v := Vlan{IfName: "eth0.100", ID: 100}
	s := v.String()
	if !strings.Contains(s, "eth0.100") {
		t.Errorf("String() missing IfName, got: %s", s)
	}
}
