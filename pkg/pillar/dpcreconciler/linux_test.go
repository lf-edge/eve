...
func TestReconcileWithEmptyArgs(test *testing.T) {
	...
	t.Expect(itemCountWithType(linux.IPv4RuleTypename)).To(Equal(1))
	t.Expect(itemCountWithType(linux.IPv6RuleTypename)).To(Equal(1))
...
func TestWireless(test *testing.T) {
	...
	rfkill := dg.Reference(linux.RFKill{})
	t.Expect(itemDescription(rfkill)).To(ContainSubstring("WLAN RF: true"))
	wpaSupplicant := dg.Reference(generic.WpaSupplicant{AdapterIfName: "wlan0"})
	t.Expect(itemDescription(wpaSupplicant)).To(ContainSubstring(
		"WPA supplicant (Wi-Fi): adapter=mock-wlan0 (wlan0)"))
	t.Expect(itemDescription(wpaSupplicant)).To(ContainSubstring("SSID=\"my-ssid\""))
	t.Expect(itemDescription(wpaSupplicant)).To(ContainSubstring("KeyScheme=WPAPSK"))
	t.Expect(itemDescription(wpaSupplicant)).To(ContainSubstring("Identity=\"my-user\""))
	t.Expect(itemDescription(wpaSupplicant)).To(ContainSubstring("Priority=0"))
	t.Expect(itemDescription(wpaSupplicant)).ToNot(ContainSubstring("my-password"))
	...
	t.Expect(status.RS.ConfigError).To(BeEmpty())
	t.Expect(itemDescription(rfkill)).To(ContainSubstring("WLAN RF: false"))
...
func TestSingleEthInterfaceWithIPv6(test *testing.T) {
	...
	eth0.DNS = []netmonitor.DNSInfo{
		{
			ResolvConfPath: "/run/dhcpcd/resolv.conf/eth0.ra",
			Domains:        []string{"test-domain"},
	...
	routes := []netmonitor.Route{
		{
			IfIndex: 1,
			Dst:     nil,
			Gw:      net.ParseIP("fe80::c225:2fff:fea2:dc73"),
			Table:   syscall.RT_TABLE_MAIN,
			Data: netlink.Route{
				LinkIndex: 1,
				Dst:       nil,
	...
	t.Expect(status.DNS.Servers).To(HaveKey("eth0"))
	t.Expect(status.DNS.Servers["eth0"]).To(HaveLen(1))
	t.Expect(status.DNS.Servers["eth0"][0].String()).To(Equal("2001:4860:4860::8888"))
