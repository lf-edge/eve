...
type MgmtDnsmasq struct {
	...
	Ports []MgmtDNSPort
	// CacheClearCounter is incremented by DpcManager to declaratively force a
	// guaranteed-clean DNS cache before each DPC verification pass; see
	// NeedsRecreate.
...
// Modify rewrites the config file and sends SIGHUP to reload it; see
// NeedsRecreate for why this is only for changes that don't need a
// guaranteed-clean cache.
...
// NeedsRecreate returns true when LogDNSQueries or CacheClearCounter change,
// forcing a full Delete+Create cycle instead of Modify()+SIGHUP for both:
//   - LogDNSQueries is a main-config-only option, not re-read on SIGHUP.
//   - CacheClearCounter changes right before a DPC verification pass (see
//     its doc comment) and needs a *guaranteed* clean cache. SIGHUP asks
//     dnsmasq to clear its cache, but delivery and dnsmasq's own handling of
//     it are asynchronous and untracked from here -- nothing here waits for
//     dnsmasq to actually finish before NIM's connectivity check runs, so a
//     stale, pre-switch cached answer could still be served. A full process
//     restart sidesteps that race entirely: Create's pm.Start() only
//     returns once the new (and therefore cache-empty) process is confirmed
//     running.
//
// Plain Ports changes (new/changed upstream DNS servers, e.g. one port at a
// time as each acquires its IP during boot) go through Modify()+SIGHUP
// instead -- restarting dnsmasq for every one of those would be excessive,
// and that path has no clean-cache requirement to protect.
//
// This does not restart dnsmasq on every periodic controller connectivity
// check: DpcManager only bumps CacheClearCounter when (re-)entering actual
// DPC verification, not on a routine successful recheck of a working DPC.
func (c *MgmtDnsmasqConfigurator) NeedsRecreate(oldItem, newItem dg.Item) bool {
	...
	return oldM.LogDNSQueries != newM.LogDNSQueries ||
		oldM.CacheClearCounter != newM.CacheClearCounter
}
...
func (c *MgmtDnsmasqConfigurator) writeConfig(m MgmtDnsmasq) error {
	if err := os.MkdirAll(mgmtDnsmasqRunDir, 0700); err != nil {
...
// on process restart (not on SIGHUP), so they change rarely (see
// NeedsRecreate).
func (c *MgmtDnsmasqConfigurator) writeMainConfig(m MgmtDnsmasq) error {
	...
	if m.LogDNSQueries {
		lines = append(lines, "log-queries")
	}
	content := strings.Join(lines, "\n") + "\n"
	if err := fileutils.WriteRename(c.configPath(), []byte(content)); err != nil {
		return fmt.Errorf("failed to write dnsmasq config %s: %w", c.configPath(), err)
	}
	return nil
}
...
func (c *MgmtDnsmasqConfigurator) writeServersFile(m MgmtDnsmasq) error {
	...
	if err := fileutils.WriteRename(c.serversPath(), []byte(b.String())); err != nil {
		return fmt.Errorf("failed to write dnsmasq servers file %s: %w", c.serversPath(), err)
	}
	return nil
}
