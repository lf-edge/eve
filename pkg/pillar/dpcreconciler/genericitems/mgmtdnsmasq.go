...
// NeedsRecreate forces a full Delete+Create instead of Modify()+SIGHUP when
// LogDNSQueries changes (main-config-only, not re-read on SIGHUP) or
// CacheClearCounter changes (SIGHUP-based cache clearing is async and
// unconfirmed; a restart guarantees an empty cache before verification
// proceeds). Plain Ports changes still go through the cheaper Modify()+SIGHUP
// path.
