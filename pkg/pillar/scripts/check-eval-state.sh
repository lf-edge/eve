...
if [ -f /persist/eval/state.json ]; then
    ...
    if command -v jq >/dev/null 2>&1; then
        ...
        for slot in IMGA IMGB IMGC; do
            tried=$(jq -r ".slots[] | select(.name==\"$slot\") | .tried // false" /persist/eval/state.json 2>/dev/null || echo "false")
            successful=$(jq -r ".slots[] | select(.name==\"$slot\") | .successful // false" /persist/eval/state.json 2>/dev/null || echo "false")
...
if [ "$scheduled_count" -eq 3 ]; then
    ...
    echo "   Next: Boot IMGA, test for 5 minutes"
