...
func CleanupStaleVMI() (int, error) {
	...
	// Get the VMI list
	vmiList, err := virtClient.VirtualMachineInstance(EVEKubeNameSpace).
		List(context.Background(), &metav1.ListOptions{})
	if err != nil {
	...
	for _, vmi := range vmiList.Items {
		err = virtClient.VirtualMachineInstance(EVEKubeNameSpace).
			Delete(context.Background(), vmi.ObjectMeta.Name, &metav1.DeleteOptions{})
		if err != nil {
