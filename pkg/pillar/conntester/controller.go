...
type ControllerConnectivityTester struct {
	...
	// Internet endpoints used to query and collect nettrace when
	// the controller is not accessible, to provide information about
	// Internet connectivity for troubleshooting purposes.
	// This list is injected and potentially updated at runtime by NIM.
	DiagInternetEndpoints []*url.URL
...
func (t *ControllerConnectivityTester) TestConnectivity(
	...
	if !airGapMode.Enabled && withNetTrace && controllerTestRV.testErr != nil {
		if _, isRTF := controllerTestRV.testErr.(*RemoteTemporaryFailure); !isRTF {
			...
			// we additionally perform Internet connectivity tests and include network
			// traces from them in the netdump for troubleshooting purposes.
			tracedReqs = append(tracedReqs, t.tryInternetEndpointsWithTracing(dns)...)
		}
	}
...
// Enable all net traces, including packet capture - ping and Internet diag requests
// are quite small.
...
// configured Internet HTTP and HTTPS endpoints and include collected traces in the output.
// This can help to determine if the issue is with the Internet access or with
// something specific to the controller.
func (t *ControllerConnectivityTester) tryInternetEndpointsWithTracing(
	...
	defer cancel()
	for _, url := range t.DiagInternetEndpoints {
		if url == nil {
			continue
		}
