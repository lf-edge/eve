		...
		for _, attempt := range sendErr.Attempts {
			...
			if errors.As(attempt.Err, &dnsErr) {
				if dnsErr.IsTemporary && !dnsErr.IsTimeout {
					...
					// Within the grace period after a cache clear, dnsmasq may
					// still be restarting or the network path still settling,
					// so give it another chance instead of failing outright.
					portMap[portLabel] = struct{}{}
				}
			}
		}
