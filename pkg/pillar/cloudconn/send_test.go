...
func makeCloudClient(t *testing.T, dns *types.DeviceNetworkStatus,
	metrics *cloudconn.AgentMetrics) *cloudconn.Client {
	...
	return cloudconn.NewClient(logObj, cloudconn.ClientOptions{
		AgentName:           "unittest",
...
func TestSendOnIntf(test *testing.T) {
	...
	client := makeCloudClient(test, &dns, agentMetrics)
...
func TestSendOnIntf_WithNetTrace(test *testing.T) {
	test.Skip("Requires https://github.com/lf-edge/eve-libs/pull/37")
	dns := getDeviceNetworkStatus(test)
	fmt.Printf("DeviceNetworkStatus %+v\n", dns)
	agentMetrics := cloudconn.NewAgentMetrics()
	client := makeCloudClient(test, &dns, agentMetrics)
...
func TestSendOnAllIntf_NoUsablePorts(test *testing.T) {
	...
	t := NewGomegaWithT(test)
	t.Expect(err).To(HaveOccurred())
	sendErr, isSendErr := err.(*cloudconn.SendError)
...
func TestVerifyAllIntf(test *testing.T) {
	...
	t.Expect(err).ToNot(HaveOccurred())
	t.Expect(rv.TracedReqs).To(BeEmpty())
	t.Expect(rv.CloudReachable).To(BeTrue()) // Cloud in this case is google.com
...
func TestVerifyAllIntf_NoUsablePorts(test *testing.T) {
	...
	t.Expect(sendErr.Attempts[1].Err.Error()).To(
		Equal("link not found for interface " + dns.Ports[1].IfName))
	t.Expect(rv.TracedReqs).To(BeEmpty())
	t.Expect(rv.CloudReachable).To(BeFalse()) // Cloud in this case is google.com
	t.Expect(rv.RemoteTempFailure).To(BeFalse())
}
