...
func TestDnsResolve(t *testing.T) {
	...
	res, errs := cloudconn.ResolveWithSrcIP(testHost, net.IP{1, 1, 1, 1}, net.IP{0, 0, 0, 0})
	if errs != nil {
		panic(errs)
	}
	if res == nil {
...
func TestDnsResolveTimeout(t *testing.T) {
	...
	res, _ := cloudconn.ResolveWithSrcIP("www.google.com", exampleCom, net.IP{0, 0, 0, 0})
	if res != nil {
...
func TestResolveWithPortsLambda(t *testing.T) {
	...
	resolverFunc := func(domain string, dnsServer net.IP, srcIP net.IP) ([]cloudconn.DNSResponse, error) {
		countCalls.Add(1)
		if !first.Swap(false) {
			time.Sleep(1 * time.Second)
			return []cloudconn.DNSResponse{}, nil
		}
		return []cloudconn.DNSResponse{
			{
				IP:  expectedIP,
				TTL: 3600,
			},
	...
	res, err := cloudconn.ResolveWithPortsLambda(
		"example.com",
		deviceNetworkStatus,
		resolverFunc,
	)
	...
	if countCalls.Load() > cloudconn.DNSMaxParallelRequests+1 {
...
func TestResolveCacheWrap(t *testing.T) {
	...
	cw := cloudconn.ResolveCacheWrap(func(domain string, dnsServerIP, srcIP net.IP) ([]cloudconn.DNSResponse, error) {
		called++
		return []cloudconn.DNSResponse{
			{
				IP:  []byte{127, 0, 0, 1},
...
func FuzzResolveWithSrcIP(f *testing.F) {
	f.Fuzz(func(t *testing.T,
		...
		cloudconn.ResolveWithSrcIPWithTimeout(domain, dnsServerIP, srcIP, 3*time.Second)
	})
}
