...
// are not affected.
//
// The CDI CR is a single, cluster-wide object, so this only works on a
// multi-node cluster because TokenFile/TLSCertFile hold the same bytes on
// every node — pillar's mgmtproxy derives them from the cluster's shared
// k3s join token instead of generating them randomly per node. Nothing to
// coordinate here: this function just reads and patches.
//
