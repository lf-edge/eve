...
func waitForBootstrapServer(ctx context.Context, apiURL, statusURL, expectedClusterID string) error {
	...
	// Bootstrap node uses a self-signed cert that is not in our trust
	// store yet — InsecureSkipVerify is correct here because the
	// cluster-UUID match below is the actual authentication.
