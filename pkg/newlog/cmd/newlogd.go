...
func handleGlobalConfigImp(ctxArg interface{}, key string, statusArg interface{}) {
	...
	if gcp != nil {
		...
		// the inline vector config is persisted separately by
		// handleVectorConfig under /persist/vector/config
		vectorConfig := gcp.GlobalValueString(types.VectorConfig)
		if err := handleVectorConfig(vectorConfig); err != nil {
			log.Errorf("handleGlobalConfigImp: handleVectorConfig failed: %v", err)
		} else {
			log.Functionf("handleGlobalConfigImp: vector config set to %s", vectorConfig)
		}
