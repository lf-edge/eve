...
# - or get the query 'edge-view' container from lf-edge/eve-edgeview:latest
# - the 'EDGEVIEW_CLIENT' variable needs to be set since the same docker image also runs on EVE edge-nodes
# shellcheck disable=SC2086,SC2046
docker run -it --rm -e EDGEVIEW_CLIENT=1 -h=$(hostname) -p 9001-9005:9001-9005 -v /tmp/download:/download lf-edge/eve-edgeview "$@"
