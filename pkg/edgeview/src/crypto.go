...
func signClientSSHKeyAuthenData(msg []byte, privateKeyPEM []byte) ([]byte, error) {
	...
	} else if block.Type != "RSA PRIVATE KEY" && block.Type != "OPENSSH PRIVATE KEY" {
	...
	if err == nil {
		signature, err = signWithRSAPrivateKey(msg, priv)
		if err == nil {
...
func verifySSHKeySignedData(msg *envelopeMsg, publicSSHKey string) bool {
	...
	case "RSA PUBLIC KEY":
		pubKey, err := x509.ParsePKIXPublicKey(block.Bytes)
		if err != nil {
			log.Errorf("verifySSHKeySignedData: ParsePKIXPublicKey failed: %v\n", err)
			return false
		}
		switch pubKey := pubKey.(type) {
		case *rsa.PublicKey:
			log.Tracef("verifySSHKeySignedData: ParsePKIXPublicKey success, verifying\n")
			return verifyWithRSAPublicKey(message, signedData, pubKey)
		default:
			log.Functionf("unsupported public key type\n")
			return false
		}
	default:
		log.Functionf("unsupported public key type\n")
...
func publicKeyToPEM(publicSSHKey []byte) ([]byte, error) {
	...
	// Convert the SSH public key to the X.509 SubjectPublicKeyInfo structure
	rsaPublicKey, ok := sshPublicKey.(ssh.CryptoPublicKey).CryptoPublicKey().(*rsa.PublicKey)
	if !ok {
		return nil, errors.New("failed to convert SSH public key to RSA public key")
	}
	x509EncodedPubKey, err := x509.MarshalPKIXPublicKey(rsaPublicKey)
	if err != nil {
...
// getEdgeviewClientPublicKey - Edgeivew client mounts the private-key path inside the container
...
// loadPrivateKey loads an EC private key from a PEM file
func loadPrivateKey(pemData string) (interface{}, error) {
	block, _ := pem.Decode([]byte(pemData))
	if block == nil || !strings.Contains(block.Type, "PRIVATE KEY") { // it can be "EC PRIVATE KEY"
