...
func (ep *AwsTransportMethod) processS3Upload(req *DronaRequest) (error, int) {
	...
	location, err := aws.S3UploadFile(nil, ep.region, ep.token, ep.apiKey,
...
func (ep *AwsTransportMethod) processS3Download(req *DronaRequest) (error, int) {
        ...
        if req.ackback {
                s := aws.NewAwsCtx(ep.token, pwd, ep.region, nil)
                if s != nil {
        ...
        err := aws.S3DownloadFile(nil, ep.region, ep.token, pwd, ep.bucket, req.name, req.objloc)
        if err == nil {
...
func (ep *AwsTransportMethod) processS3Delete(req *DronaRequest) error {
        var err error
        s3ctx := aws.NewAwsCtx(ep.token, ep.apiKey, ep.region, nil)
        if s3ctx != nil {
