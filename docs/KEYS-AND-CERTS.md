...
In order to satisfy all of the security requirements discussed in [SECURITY](SECURITY-ARCHITECTURE.md), and work with the constraints of trusted platform modules (TPMs) when it comes to different key usage, EVE requires several different keys and associated certificates.
...
|----------|---------|------|----------|-----------|
| Device key   | Prove the identity of the device |  ECC (P-256) | TPM or /config/device.key.pem | [Identity of EVE](SECURITY-ARCHITECTURE.md#identity-of-eves-instance) |
...
| Endorsement cert | Needed for some use of vTPM | Depends on TPM | Generated by TPM | [Identity of EVE](SECURITY-ARCHITECTURE.md#identity-of-eves-instance) |
| Vault Encryption | Lock/unlock vault |  | Sealed to TPM PCRs  | [Encrypted Application and User Storage](SECURITY-ARCHITECTURE.md#encrypted-application-and-user-storage) |
| Vault Encryption | Backup for upgrades |  | Encrypted and sent to controller  | [Encrypted Application and User Storage](SECURITY-ARCHITECTURE.md#encrypted-application-and-user-storage) |

## Controller certificates used by EVE

| Certificate | Purpose | Type | Location | Reference |
|-------------|---------|------|----------|-----------|
| Root of trust | Validate legitimate controller | Any | /config/root-certificate.pem | [Trusting controller](SECURITY-ARCHITECTURE.md#eve-trusting-its-controller) |
| Signing cert | Signing of API messages | Any | Only in memory | Populated using the EVE API. See [SIGNING](https://github.com/lf-edge/eve-api/tree/main/OBJECT-SIGNING.md) |
| ECDH cert | API object encryption | ECC (P-256) | Only in memory | [Config object encryption](OBJECT-LEVEL-ENCRYPTION.md) |
| Intermedia certs | Signing and object encryption | Any | Only in memory | If needed to validate chain to the root of trust |
