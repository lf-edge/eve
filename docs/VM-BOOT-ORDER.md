...
EVE uses a patched OVMF firmware with EveBootOrderLib that reads
boot order configuration via QEMU's fw_cfg mechanism. This approach is necessary
because EVE's USB passthrough uses dynamic hotplug—USB devices are attached to
running VMs at runtime via QMP, and QEMU's standard `bootindex` parameter only
works for devices configured at VM startup.
