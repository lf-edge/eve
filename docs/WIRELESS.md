...
Wi-Fi adapters are configured via the [WirelessConfig](https://github.com/lf-edge/eve-api/blob/main/proto/config/netconfig.proto)
structure defined in the EVE API.

Key points:

* Each adapter can have multiple `WifiConfig` entries, e.g., one for 2.4GHz and another
  for 5GHz SSIDs, each with a different priority
* WifiConfig supports WPA/WPA2 Personal (PSK) and Enterprise (EAP) authentication.
* Credentials can be provided either in cleartext form or [fully encrypted](./OBJECT-LEVEL-ENCRYPTION.md).
* For WPA/WPA2 Personal or Enterprise networks using a password, the password must be
  supplied in pre-hashed form, as generated by `wpa_passphrase` (`PBKDF2-HMAC-SHA1` hash
  derived from the plaintext password and the SSID).

### WPA Supplicant

EVE uses [wpa_supplicant](https://wiki.archlinux.org/title/wpa_supplicant)
for Wi-Fi authentication. The NIM microservice translates `WifiConfig` entries into
per-interface `wpa_supplicant` configuration files and manages a dedicated `wpa_supplicant`
instance for each interface. This enables support for multiple Wi-Fi network ports,
allowing devices to connect to several Wi-Fi networks simultaneously.
