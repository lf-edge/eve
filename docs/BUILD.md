...
2. tar these two dependencies together and stream to [makeflash.sh](../makeflash.sh)
3. Call `./makeflash.sh -C installer.raw "efi conf_win installer inventory_win"`
4. `makeflash.sh` creates an empty image of the target size at the target path, and then passes control to a docker container from the image generated by `pkg/mkimage-raw-efi`. It _does_ pass on the names of the partitions to be created, limiting it to `efi`, `conf_win`, `installer`, `inventory_win`. There is no need for `imgb` or `persist` partitions for an installer image that will not be persisting data, and will not be updating its root filesystem.
5. `mkimage-raw-efi`:
    0. Calculates required size of the resulting image if '-C' option is provided
    1. Extracts the contents of the tar stream to `/parts/`
    2. Creates a partition for each of `efi`, `conf_win`, `installer`, `inventory_win`
    3. Populates each partition with its appropriate contents:
        * `efi`: contents of `/EFI/BOOT/` from `/parts/rootfs.img`
        * `installer`: contents of `/parts/rootfs.img`
...
Note that once you flash `installer.raw` on the installer media, such as USB drive or SD card, since the `conf_win` and `inventory_win` partitions are Windows Data partitions, most operating systems will recognize them and allow you to mount them. This allows you to update configuration on the installer media between installs.
...
* modifiers are located in `images/modifiers` and structured as following:
  ...
  * `images/modifiers/platform/<platform name>/` - platform-specific modifiers. A platform may require separate modifiers to be applied to `rootfs.yml.in` and `installer.yml.in`. In this case the platform-specific modifiers are located in `images/modifiers/platform/<platform name>/installer.yq` (applied to the installer) while the rootfs uses `images/modifiers/platform/<platform name>/<platform name>.yq`. If rootfs and installer share the same modifiers, the `images/modifiers/platform/<platform name>/` directory may contain only a single file, e.g. `images/modifiers/platform/<platform name>/<platform name>.yq`.
...
1. The Makefile runs `tools/compose-image-yml.sh -b images/rootfs.yml.in -v "$(ROOTFS_VERSION)-$(HV)-$(ZARCH)" -h $(HV) -o images/out/rootfs-$(HV)-$(PLATFORM).yml.in images/modifiers/hv/$(HV).yq images/modifiers/platform/$(PLATFORM)/[installer | <platform-name>].yq`, i.e. the utility [compose-image-yml.sh](../tools/compose-image-yml.sh), passing it:
   ...
   * the hypervisor
   * the output file, specifically `images/out/rootfs-$(HV)-$(PLATFORM).yml`
...
1. The Makefile runs `./tools/parse-pkgs.sh images/out/rootfs-$(HV)-$(PLATFORM).yml.in > images/out/rootfs-$(HV)-$(PLATFORM).yml`, i.e. the utility [parse-pkgs.sh](../tools/parse-pkgs.sh), passing it as an input the HV-specific template generated in the previous step `rootfs-$(HV).yml.in`, and saving the output to the final `images/out/rootfs-$(HV)-$(PLATFORM).yml` file. In addition, the variable `KERNEL_TAG` is passed as an environment variable.
...
|---|---|---|
|EFI|boot via grub|`makeraw.sh`|
|installer|Root partition|`rootfs_installer.img` from linuxkit build|
|conf_win|Config data|`config.img` from `tools/makeconfig.sh`|
|inventory_win|Inventory data (empty)|created by `mkimage-raw-efi`|
