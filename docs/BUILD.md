...
2. Call the script to create the image: `./maketestconfig.sh conf config.img`. This will:
    ...
    2. stream the contents of the tar format into a docker container from the image generated by `pkg/mkconf`. `mkconf` is an image that contains the `make-config` script and all of `/opt/zededa/examples/config` from [ztools](https://github.com/lf-edge/eve/pkg/pillar).
...
* [mkconf](../pkg/mkconf) - combine the input tar stream with defaults in `/conf/` from `lfedge/eve-ztools` into a new container image in `/`. Create a FAT32 disk image from it.
...
```yml
FROM lfedge/eve-ztools@sha256:4a6d0bcfc33a3096398b4daa0931f9583c674358eeb47241e0df5f96e24c0110 as lfedge
```

We may want to update to the latest current version of `lfedge/eve-ztools`. 
...
6. Modify all stdin to replace any tags with the appropriate named and hashed packages, e.g. `STRONGSWAN_TAG` to `lfedge/eve-strongswan@sha256:abcdef5678`
