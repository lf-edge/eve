...
Once the gzip log files are uploaded to the cloud, the gzip files are still available on the device in the circular buffer in /persist/newlog/keepSentQueue directory (up to the maximum of 1000 gzip files). For any log files are still waiting to be uploaded, they are in the '/persist/newlog/devUpload' and '/persist/newlog/appUpload' directories. EVE developers who have enabled ssh to the device for debugging purposes can look the log entries at in those directories by using "zcat" utility.

User can use config-properties to set a log file maximum quota in Mbytes on the device, using the 'newlog.gzipfiles.ondisk.maxmegabytes' config-item, the default is unlimited (except for the above system applied limitation), the range is within (10, 4294967295) Mbytes. When the maximum quota is reached on the device, the system will remove the gzip log files starting with the oldest timestamp file until the total log file size on the device is below the maximum quota. Since the already uploaded files in 'keepSentQueue' directory are older than the ones in 'appUpload' and 'devUpload' directories, those files in 'keepSentQueue' will be removed first during this periodical quota maintenance.
