...
## What EVE loses without each of these

The recommendations below are not a wish list: each one underwrites a specific guarantee in the [security architecture](SECURITY-ARCHITECTURE.md), and EVE has no way to enforce or detect the absence of any of them. A deployment's real posture is the weaker of that document and the platform underneath it. The sections that follow give the reasoning; this table gives the consequence.
...
| Authenticated firmware update with rollback protection | Persistence below EVE. An authentic but vulnerable BIOS can be reinstalled to regain access that no EVE update will remove |
| Protected or removed debug interfaces | All of it. JTAG-class access bypasses every control described in the security architecture |
| Memory encryption | Protection of keys in use. The vault key and application data live in RAM while the device runs, and cold-boot techniques recover them after power-off |
