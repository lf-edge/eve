...
One other part of the cross-flavor conversion belongs here because of its lasting
effect on the converted device: on a ZFS device the vault also moves from a
filesystem dataset to the EVE-k zvol layout. That vault is sized from the pool
space free at conversion time, so it stays smaller than the vault a fresh EVE-k
install of the same hardware would create, and the space the old vault frees
returns to the pool rather than growing the vault. A pool that cannot hold a
second copy of the vault declines the conversion. The mechanism is in
`pkg/pillar/docs/vaultmgr.md`.

This conversion is only attempted when the controller pushes a cross-flavor
base-OS update, asks for that image to be activated, and the device is in a
state where the conversion is allowed. A cross-flavor image that is merely
pre-staged for later activation is downloaded and otherwise left alone: the
repartition reboots the device, and on the shrink path shrinks `/persist`, so it
waits for the request to actually switch flavor. Repartition is the prerequisite
step; the normal A/B install runs afterwards.
...
- **baseosmgr** (`pkg/pillar/cmd/baseosmgr`) — the online orchestrator. At the
  ...
  and verified and once the controller asks to activate it, but before the A/B
  partition write, it drives `diskconvert` for one conversion step and records
  progress on `BaseOsStatus`. It also reads the failure marker (below) to report
  a declined conversion and to gate retries.
...
**How the offline pass reboots.** The reboot-to-apply and abort paths reset the
device with a magic-sysrq write (`echo b > /proc/sysrq-trigger`) after a `sync`,
not with a graceful `reboot`. storage-init is an onboot container with its own
PID namespace, so a graceful `reboot` there signals that namespace's init rather
than the host and has no effect; the sysrq write acts on the host kernel, and is
the mechanism `pkg/kdump` uses for the same reason. Both reboot forms are kept as
fallbacks in case sysrq is unavailable, and if none of them takes effect
storage-init halts rather than proceeding with a mid-resize disk. Resetting hard
costs nothing here: `/persist` is unmounted, the CONFIG writes are synced and
unmounted before the reset, and the GPT is either committed or not.
...
- `docs/EVE-K.md`, `docs/HYPERVISORS.md` — the EVE-k flavor this conversion
  targets.
- `pkg/pillar/docs/vaultmgr.md` — the vault's move to the EVE-k zvol layout.
