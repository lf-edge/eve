...
free tail, copies into them, and only then renames/reindexes them back to the
canonical labels and removes the originals — so a power loss at any point leaves
either the old layout or the new one bootable, never a half-built disk.
...
1. **Online, in baseosmgr:** the check returns `shrink`. Before doing anything
   ...
   controller signing certs, and the `/persist/certs/` attestation/decryption
   keys that cannot be re-derived once the filesystem is wiped) to the CONFIG
   partition, then writes the shrink flag file (carrying the target size)
