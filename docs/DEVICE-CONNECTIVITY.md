...
it performs, including `/ping` request to the controller and `GET` requests to Internet endpoints
over HTTP/HTTPS when the controller is unreachable to assess the state of Internet connectivity.
Previously, these requests were hardcoded to `www.google.com`, but they can now be configured
via the global configuration properties `diag.probe.internet.http.endpoint` and
`diag.probe.internet.https.endpoint`. These probes are used only for diagnostics and
can be disabled by setting the properties to an empty string.
...
- `/ping` request done by NIM to verify connectivity for the *latest* DPC (testing of older DPCs
 ...
 performs Internet connectivity tests and includes network traces from them in the netdump
 for troubleshooting purposes. The Internet endpoints used are configurable via
 `diag.probe.internet.http.endpoint` and `diag.probe.internet.https.endpoint` and can be
 disabled by setting them to empty strings (default is `www.google.com` for both).
