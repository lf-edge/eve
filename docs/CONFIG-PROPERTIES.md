...
| cluster.dnid.backupnode.threshold | integer | 600 | 60 | 86400 | seconds an app's designated node must have been unhealthy before another cluster node may act on that app in its place. The effective delay adds the Kubernetes node-monitor grace period, and is floored at the `eve-app-op` lease duration, so a value below that lease is bounded by lease handover rather than by this setting. Failback additionally requires `kubernetes.vmi.deschedule.events` to include `boot`; without it an app that moved to another node keeps running there. |
