...
| edgeview.authen.publickey | string | "" | Specifies SSH RSA public keys used for Edgeview client command authentication. The user issuing an Edgeview command must sign it with their SSH private key, and the device verifies the signature using one of the public keys provided in this configuration. Multiple public keys should be separated by newline characters. |
