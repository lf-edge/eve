...
| cluster.dnid.backupnode.threshold | integer | 600 | 60 | 86400 | seconds an app's designated node must have been unhealthy before another cluster node may act on that app's config via Activate, Deactivate, Delete, Purge, Restart. The effective delay adds the Kubernetes node-monitor grace period, and is floored at the `eve-app-op` lease duration, so a value below that lease is bounded by lease handover rather than by this setting. This is not linked to failover or failback, the latter of which is controlled via kubernetes.vmi.deschedule.events. |
...
| igpu.gop | string | "" | - | - | Filename (basename only) of a proprietary Intel GOP Option ROM placed under `/persist/vault/gop/`. Used for Intel iGPU passthrough to provide a pre-OS UEFI framebuffer. Empty (default) or a missing file falls back to the bundled open-source `igd.rom` (IgdAssignmentDxe only — OS display works but no pre-OS framebuffer). Path separators, `..`, and absolute paths are rejected to prevent directory traversal. See [INTEL-IGPU-PASSTHROUGH.md](INTEL-IGPU-PASSTHROUGH.md) for details. |
