...
| diag.probe.internet.http.endpoint | string | www.google.com | Internet endpoint (URL, bare IP is also accepted) queried over HTTP to assess the state of Internet connectivity whenever the controller is not reachable. Used only for diagnostics (no functional impact). Set to an empty string to disable. |
| diag.probe.internet.https.endpoint | string | www.google.com | Internet endpoint (URL, bare IP is also accepted) queried over HTTPS to assess the state of Internet connectivity whenever the controller is not reachable. Used only for diagnostics (no functional impact). Set to an empty string to disable. |
