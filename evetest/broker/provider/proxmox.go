...
func (p *ProxmoxProvider) PowerOnDevice(ctx context.Context, name string) error {
	...
	if dev.netbootMTUCapped && dev.netbootCompleted {
		options, nicArgs := p.nicOptions(dev, false)
...
// possible afterward.
func (p *ProxmoxProvider) nicOptions(dev *proxmoxDevice, mtuCapped bool) (
	...
	for i, iface := range dev.ifaces {
		...
		nicArgs += fmt.Sprintf(
			" -device pcie-root-port,id=evrp%d,bus=pcie.0,addr=0x%x,chassis=%d"+
				...
				"host_mtu=%d,rx_queue_size=1024,tx_queue_size=256",
			i, 0x10+i, 200+i,
			iface.vnet, i, tap,
			iface.vnet, i, i,
			strings.ToUpper(iface.mac.String()), hostMTU)
	}
...
func (p *ProxmoxProvider) buildVMOptions(dev *proxmoxDevice, diskRefs []string,
	...
	// nicOptions for the uplink-vs-xconnect split and the MTU handling
	// (including the NetworkBootFallback cap that PowerOnDevice lifts
	// again after the one network boot).
	dev.baseArgs = extraArgs
	nicOpts, nicArgs := p.nicOptions(dev, spec.NetworkBootFallback)
