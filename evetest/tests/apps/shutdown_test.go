...
// all app instances" operation and recovery from it: the app must reach
// HALTED as the device passes through DEVICE_STATE_PREPARING_POWEROFF and
// DEVICE_STATE_PREPARED_POWEROFF, and it must come back to RUNNING -- along
// with its network instance -- once the device is rebooted, even if
// connectivity is lost for a while right after the reboot starts.
//
// Network model
// -------------
...
//     for several minutes, since evetest's AdminUp toggle is a
//     provider-agnostic SDN mechanism. Restore AdminUp afterwards.
//  4. Confirm recovery: the app reaches RUNNING again, the network instance
//     reaches ZNETINST_STATE_ONLINE again, and the app is reachable over SSH
//     again.
...
// Test params
// -----------
//   - HYPERVISOR (defaults to KVM). Under Kubevirt the test waits for the
//     cluster node to become ready before deploying the app.
//
// Suite placement
// ---------------
//   - TestAppsSuite.
