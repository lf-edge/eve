...
// unchanged. Directory listings and path checks come from the framework
// (EdgeDevice.ListDirEntries, EdgeDevice.PathExists); nothing here shells out on
// its own.
//
...
//
// A directory EVE has not created yet simply contributes nothing
// (EdgeDevice.ListDirEntries), but a listing that could not be run is failed
// rather than skipped: these paths feed an assertion that nothing unexpected
// exists, which an unreachable device would otherwise satisfy vacuously.
func listVolumeArtifacts(g Gomega, dev *evetest.EdgeDevice) []string {
	var paths []string
	for _, dir := range volumeDirs {
		entries, err := dev.ListDirEntries(dir)
		g.Expect(err).ToNot(HaveOccurred(), "listing volume directory %s", dir)
...
//
// This carries the most weight on the kvm path. A duplicate workload is
// structurally impossible there (no cluster-side object outlives the node), so
...
func assertNoStaleVolumeArtifacts(g Gomega, dev *evetest.EdgeDevice) {
	...
	for _, p := range listVolumeArtifacts(g, dev) {
...
func assertVolumeArtifactGone(g Gomega, dev *evetest.EdgeDevice, target string) {
	exists, err := dev.PathExists(target)
	g.Expect(err).ToNot(HaveOccurred(), "checking whether %q still exists", target)
