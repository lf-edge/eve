...
const (
	...
	// derivation the vault was created with. Overwriting it with the wrong
	// derivation is the injected fault.
	vaultKeyModeDir  = "/persist/status/vaultmgr/VaultConfig"
	vaultKeyModeFile = vaultKeyModeDir + "/global.json"
	// types.VaultConfig with TpmKeyOnly false: the pre-7.10.0 merged
	// derivation, which cannot open a vault created TPM-key-only.
	vaultKeyModeMerged = `{"TpmKeyOnly":false}`
...
// TestVaultKeyModeRecovery verifies that a device recovers on its own when the
// file recording which key derivation its vault was created with names the
// wrong one.
//
...
// pre-7.10.0 form). A device can end up with the wrong bit recorded. An EVE
// that lost the file to an e2fsck repair of a torn /persist guessed the mode
// from whether the vault existed -- which for any existing vault answers
// "merged" -- and wrote that guess out before anything tried to unlock with
// it; and the vault recreate path keys its replacement TPM-key-only whatever
// the old vault used. Either way the recorded mode does not open the vault.
//
// The TPM unseal still succeeds and only the filesystem's own key check
// refuses, so the failure reads as a broken seal: the controller-key fallback
// re-seals and fails identically, and the device parks in maintenance mode with
// the vault locked. Treating the recorded mode as authoritative made that
// permanent.
//
// Fault injection
// ---------------
// The persisted VaultConfig is overwritten with the merged derivation, then a
// controller-requested reboot. Writing the wrong bit rather than removing the
// file is what keeps the fault reachable: a device with no recorded mode at all
// starts from TPM-key-only and opens this vault on the first attempt.
//
...
//     with the merged key the fault below would record the mode already in use
//     and the test would prove nothing.
//  2. A marker file goes into the vault, to tell a recovered vault from a
//     recreated one.
//  3. rebooted-with-wrong-key-mode: the fault is confirmed to have landed before
...
//     required to grow.
//  4. The logs must show the recorded mode failing and the other derivation
...
//     a device reporting an open vault; the marker must read back; the corrected
//     mode must be persisted; and the device must not be in maintenance mode.
//  5. rebooted-with-corrected-key-mode: a second reboot must open the vault on
//     the recorded mode with no retry, so the correction is what survives.
//
// The ZFS variant exercises a different vault handler with the same fault,
// retrying only the `zfs load-key`.
//
// Network model
// -------------
...
func TestVaultKeyModeRecovery(test *testing.T) {
	...
	t.Expect(keyMode.TpmKeyOnly).To(BeTrue(),
		"this test needs a vault created TPM-key-only; against one created with "+
			"the merged key the fault below would record the derivation already "+
			"in use and nothing would be exercised")
	...
	// Baselines: these messages recur across boots, so only growth after the
	// fault is evidence.
	const (
		msgWrongFromDisk = "Vault config inited with tpmkeyonly false"
		msgRetry         = "Vault did not open with tpmKeyOnly=false"
		msgOpened        = "Vault opened with tpmKeyOnly=true"
		msgRecorded      = "Publishing Vault Config with tpmKeyOnly true"
		msgFromDisk      = "Vault config inited with tpmkeyonly true"
	)
	base := map[string]int{}
	for _, msg := range []string{msgWrongFromDisk, msgRetry, msgOpened, msgRecorded} {
		base[msg] = newlogCount(t, device, msg)
	}

	log.Infof("Recording the wrong vault key mode in %s...", vaultKeyModeFile)
	_, stderr, err := device.RunShellScript(
		"echo '"+vaultKeyModeMerged+"' > "+vaultKeyModeFile, shellCmdTimeout, 0)
	t.Expect(err).ToNot(HaveOccurred(),
		"failed to write %s (stderr: %s)", vaultKeyModeFile, stderr)
	device.SyncDisks()
	recorded, err := device.ReadFile(vaultKeyModeFile)
	t.Expect(err).ToNot(HaveOccurred())
	t.Expect(strings.TrimSpace(string(recorded))).To(Equal(vaultKeyModeMerged),
		"%s still records the working mode, so the reboot below would be an "+
			"ordinary one and every assertion after it would pass without the "+
			"fault", vaultKeyModeFile)

	log.Infof("Rebooting the device with the wrong vault key mode recorded...")
	device.RequestReboot(true)
	evetest.Checkpoint("rebooted-with-wrong-key-mode")

	waitForVaultOpen(t, device, filesystem, "after the wrong key mode was recorded")

	// The fault landed: vaultmgr read the merged derivation off disk on this
	// boot.
	expectNewlogGrew(t, device, msgWrongFromDisk, base[msgWrongFromDisk],
		"vaultmgr must read the merged derivation the fault recorded")
	// The recorded mode did not open the vault, and the other derivation did.
	expectNewlogGrew(t, device, msgRetry, base[msgRetry],
		"the recorded mode must fail and be retried, not be trusted")
	expectNewlogGrew(t, device, msgOpened, base[msgOpened],
		"the vault must open with the derivation the record got wrong")
	expectNewlogGrew(t, device, msgRecorded, base[msgRecorded],
		"the derivation that opened the vault must replace the recorded one")
	...
	t.Eventually(func() error {
		...
		"the mode that opened the vault must be persisted")
	t.Expect(keyMode.TpmKeyOnly).To(BeTrue(),
		"the persisted mode must be the one that worked, not the recorded one")
	...
	stickyBase := map[string]int{
		msgWrongFromDisk: newlogCount(t, device, msgWrongFromDisk),
		msgRetry:         newlogCount(t, device, msgRetry),
		msgFromDisk:      newlogCount(t, device, msgFromDisk),
	}
	log.Infof("Rebooting the device again, now with the key mode corrected...")
	device.RequestReboot(true)
	evetest.Checkpoint("rebooted-with-corrected-key-mode")
	...
	// about the vault config, and what it logged is the corrected mode.
	expectNewlogGrew(t, device, msgFromDisk, stickyBase[msgFromDisk],
		"vaultmgr must read the corrected mode from disk")
	t.Expect(newlogCount(t, device, msgWrongFromDisk)).To(
		Equal(stickyBase[msgWrongFromDisk]),
		"the correction must survive the reboot")
	t.Expect(newlogCount(t, device, msgRetry)).To(Equal(stickyBase[msgRetry]),
		"a mode that opens the vault must not cost a second attempt")
