...
const (
	...
	// conversionUpgradeTimeout is what UpgradeEVE gets for the hops of a
	// conversion run. The framework default is sized for an ordinary base-OS
	// upgrade and a conversion lands close enough to it that host load decides
	// the verdict.
...
// TestKvmToKRepartition drives 16.6.0-kvm (small) → current EVE-kvm (same
// geometry, lands the conversion code) → current EVE-k (cross-flavor ⇒ offline
// shrink+grow), keeping a container app across the conversion.
//
// Parameters:
...
//     writing a pre-built live image to it whole. The two differ in one way that
//     matters here: an installer-written ESP carries a zero-length
//     boot/.boot_repository, and the offline grow relocates the ESP by copying
//     its FAT32 contents, which go-diskfs can only read with
//     diskfs/go-diskfs#419 ("invalid start cluster: 0" without it). A live-image
//     ESP has no such file, so only USE_INSTALLER=true covers that path; run it
//     against an EVE image built with the fix, and expect a longer setup because
...
func TestKvmToKRepartition(test *testing.T) {
	...
	device.UpgradeEVE(convVersion, targetHypervisor, evetest.BaseOSDatastoreHTTP,
		true, false, conversionUpgradeTimeout)
	conversionOK = true
	...
	for _, line := range strings.Split(lsblk, "\n") {
		...
		for _, field := range strings.Fields(line) {
			if strings.HasPrefix(field, "SIZE=") {
				var n int64
				_, _ = fmt.Sscanf(strings.Trim(strings.TrimPrefix(field, "SIZE="), `"`), "%d", &n)
				return n
			}
		}
	}
