...
// shrink+grow), keeping a container app across the conversion. The device is
// provisioned from the LIVE image, the way eden brings one up.
//
// Parameters:
...
func TestKvmToKRepartition(test *testing.T) {
	runKvmToKRepartition(test, evetest.CreateFromScratchWithLiveImage)
}

// TestKvmToKRepartitionFromInstaller runs the same conversion against a boot
// disk the EVE installer laid out, instead of a pre-built live image written to
// the disk whole. That distinction is what the variant is for: an installer-written ESP
...
//
// Same parameters as TestKvmToKRepartition. Expect a longer setup: the
// installer VM boots and writes the disk before the test's own device is up.
func TestKvmToKRepartitionFromInstaller(test *testing.T) {
	runKvmToKRepartition(test, evetest.CreateFromScratchWithInstaller)
}

// runKvmToKRepartition is the body shared by both variants. provisionPolicy
// selects how the device's boot disk comes into existence; everything after
// Setup is identical, since the conversion sequence does not depend on it.
func runKvmToKRepartition(test *testing.T, provisionPolicy evetest.ExistingEdgeDeviceReusePolicy) {
