...
// TestKvmToKRepartitionAppVolumeFromInstaller for the installer-written variant.
func TestKvmToKRepartitionAppVolume(test *testing.T) {
	runKvmToKRepartitionAppVolume(test, evetest.CreateFromScratchWithLiveImage)
}

// TestKvmToKRepartitionAppVolumeFromInstaller takes the same corruption
...
//
// Same parameters as TestKvmToKRepartitionAppVolume. Expect a longer setup:
// the installer VM boots and writes the disk before the test's own device is
// up.
func TestKvmToKRepartitionAppVolumeFromInstaller(test *testing.T) {
	runKvmToKRepartitionAppVolume(test, evetest.CreateFromScratchWithInstaller)
}

// runKvmToKRepartitionAppVolume is the body shared by both variants.
// provisionPolicy selects how the device's boot disk comes into existence;
// everything after Setup is identical.
func runKvmToKRepartitionAppVolume(test *testing.T,
