...
// TestKvmToKRepartitionAppVolume checks whether the EVE-kvm→EVE-k offline
// boot-disk shrink corrupts an application data volume when it is interrupted
// part-way, and whether the corruption is detectable.
//
// It follows the same conversion sequence as TestKvmToKRepartition — that
// ordering and those readiness gates are load-bearing, see the comments there —
// and swaps the plain container app for volverify with a blank data volume. The volume is filled
...
//
// Parameters: as TestKvmToKRepartition, plus
//
...
//
//   - USE_INSTALLER: lay the boot disk out with the EVE installer instead of
//     writing a pre-built live image to it whole. An installer-written ESP
//     carries a zero-length boot/.boot_repository, which the offline grow's
//     FAT32 copy can only read with diskfs/go-diskfs#419 ("invalid start
//     cluster: 0" without it); a live-image ESP has no such file, so only
//     USE_INSTALLER=true covers that path, and expect a longer setup because
//     the installer VM boots and writes the disk before the device is up.
func TestKvmToKRepartitionAppVolume(test *testing.T) {
	...
	// The shared parameters treat 0 as "framework default", which is below what
	// this conversion needs, so an unset value resolves to the floor instead.
	...
	provisionPolicy := evetest.CreateFromScratchWithLiveImage
	if evetest.GetTestParameter[bool](useInstallerParamKey) {
		provisionPolicy = evetest.CreateFromScratchWithInstaller
	}
	...
	device.UpgradeEVE(convVersion, evetest.HypervisorKVM, evetest.BaseOSDatastoreHTTP,
		true, false, conversionUpgradeTimeout)
	assertSmallGeometry(t, device)
...
// devsideVerifyScript locates the carried-over volume on the device and hands it to
// the verdict script the volverify app ships (testapps/volverify/scripts/), which is
// also runnable by hand inside the app during a stuck upgrade.
//
