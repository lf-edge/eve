...
// starting the device on the build under test unless the operator named a
// version themselves.
func resolveCrossFlavorParams(t Gomega) deviceParams {
	p := resolveDeviceParams(t)
	if p.initialVersion == "" {
		p.initialVersion = p.targetVersion
	}
	return p
}
...
// version, so that the flavor is the only thing that changed.
