...
//  2. Prepare power off Node1 and the tie-breaker -- not modeled: evetest
//     has no trigger for EVE's graceful "prepare for power off" (see step
//     3's comment for why a hard PowerOff substitutes here).
//  3. Shutdown 3x nodes -- hard PowerOff on all three, combined with step
//     2, since the mechanism under test does not depend on shutdown
//     gracefulness.
//  4. Shutdown the datastore -- an SDN firewall rule drops traffic to
//     evetest's image server, left in place for the rest of the test.
//  5. Disable Node2's cluster interface -- AdminUp=false on its
//     cluster-interconnect port, applied together with step 4.
...
func TestClusterPVCRedownload(test *testing.T) {
	...
	// Steps 2+3: power off all three nodes.
	...
	for i := range contentTrees {
		if contentTrees[i].ContentSha256 == image.sha256 {
			vm1Tree = &contentTrees[i]
			break
		}
	}
	t.Expect(vm1Tree).NotTo(BeNil(),
		"no ContentTreeStatus on %s matches vm1's image SHA256 %s", devName[0], image.sha256)
