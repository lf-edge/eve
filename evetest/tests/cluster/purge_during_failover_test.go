...
const (
	...
	// its node is powered off. Dominated by node-not-ready detection and by
	// KubeVirt's own control plane recovering: detaching the old workload
	// scales the previous VMIRS down, which goes through the
	// virtualmachinereplicaset validating webhook, so the reschedule cannot
	// finish until virt-api is serving again on a surviving node. Observed
	// between 7m47s and over 10 minutes, the long tail being runs where
	// virt-api had to come back first.
	failoverRescheduleTimeout = 20 * time.Minute

	// failoverPurgeIssueTimeout bounds resolving which node hosts the app so
	// the purge can be addressed to it. The purge counter is then pushed to
	// every device without waiting; failoverPurgeObservedTimeout covers the
	// purge itself.
	failoverPurgeIssueTimeout = 5 * time.Minute

	// failoverPurgeObservedTimeout bounds the whole purge as seen from the
	// hosting node: the old generation torn down through to the new
	// generation's VMIRS existing.
	failoverPurgeObservedTimeout = 15 * time.Minute
...
const (
	...
	// failoverThresholdPin is what this test pins that setting to: several
	// times the longest it can hold the node down (failoverRescheduleTimeout
	// plus failoverPurgeObservedTimeout), and inside the range the setting
	// accepts.
...
//  5. purge-issued: EdgeCluster.PurgeApplication bumps the purge counter on
//     every device, including the powered-off node 1 - EdgeDevice.ApplyConfig's
//     push does not require device reachability - and returns without waiting.
//     The controller cannot be asked whether the purge ran: with the
//     designated node down no node uploads this app's state at all, so the
//     PURGING and HALTING it would be waited on for are never reported. See
//     the comment at the call.
//  6. purge-end-state-asserted: exactly one VMIRS, named for the NEW
//     generation, read from the node that now hosts the app - the purge having
//     finished and the end state, in one wait. No volume or
...
func TestVMAppPurgeDuringFailover(test *testing.T) {
	...
	// search for the node the purge is addressed to.
	//
	// Not waitUntilPurged: that waits on the app's state as the CONTROLLER
	// sees it, and no node uploads this app's state while its designated node
	// is down. zedmanager elects one reporter - the node the pod is scheduled
	// on, else the designated node - and a purge renames the pod it looks for
	// (GetAppKubeNameWithPurge embeds the purge counter), so every node finds
	// no pod and defers to a node that is powered off. PURGING and HALTING
	// happen where nothing uploads them.
	cluster.PurgeApplication(appUUID, evetest.BumpVolumeGeneration, false,
		failoverPurgeIssueTimeout, devName[0])
	evetest.Checkpoint("purge-issued")

	// The new generation's VMIRS existing is both the purge having finished
	// and the end state asserted here, so one wait covers both. Read from the
	// hosting node, which stays truthful while no node is reporting.
	wantCounter := baselineCounter + 1
	t.Eventually(func(g Gomega) {
		assertExactlyOneVMIRSAtGeneration(g, failoverHost, appUUID, appDisplayName, wantCounter)
	}, failoverPurgeObservedTimeout, failoverPollInterval).Should(Succeed())
