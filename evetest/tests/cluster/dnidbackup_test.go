...
//     affinity), and vm3 (DNID edge-dev3, Preferred affinity).
//
...
//
//  7. Purge vm2, wait running, while still on the backup, with
//     evetest.KeepVolumeGeneration: it recreates the domain but leaves the
//     volume alone, so this exercises the activation gate again under a
//     different app command, not volumemgr. A purge that also bumped the
//     volume's generation would additionally need volumemgr's own
//     backup-DNID bypass, which does not exist yet -- and whether the real
//     controller's purge action even bumps it is unconfirmed (see
//     VolumeGenerationPolicy). Issued through cluster.PurgeApplication, not
//     dev1.PurgeApplication: the latter only bumps edge-dev1's own tracked
//     config, leaving edge-dev3's purge counter stuck at 0 -- indistinguishable,
//     once edge-dev3 reboots for step 9, from edge-dev3 never having purged at
//     all -- so it keeps hunting for a pod at the old generation forever.
//
//  8. Delete vm3 entirely while edge-dev3 is still down, and confirm both
//     its app info and its volume info actually reach INVALID. vm3 stays
//     Preferred-affinity and Activated the whole test, so by this point
//     the same backup-DNID ticker that activates vm2 has very likely
//     already activated vm3 on a backup node too -- this deletes an app
//     actually running on a peer, not merely configured to run there,
//     confirming the info/volume reporting and the domain teardown both
//     converge correctly once neither the app's config nor its domain
//     exists on the DNID node anymore. Delete has no other coverage in
//     this test: vm2 is only ever activated, deactivated, and purged,
//     never deleted.
//
//     Deliberately not covered here: a Required-affinity app deleted
//     while it has never run anywhere reachable and never will. That
//     case exposes a real gap -- no node ever holds the in-memory
//     domainmgr state needed to actually delete the Kubernetes VMIRS, so
//     it leaks, orphaned and endlessly retried by Kubernetes' own
//     ReplicaSet controller -- tracked separately in the DNID rework
//     plan, not exercised by this test for now.
//
...
func TestDNIDandBackupDNID(test *testing.T) {
	...
	// Step 7.
	cluster.PurgeApplication(vm2UUID, evetest.KeepVolumeGeneration, true, clusterTimeout)
	...
	// devName[2] (edge-dev3) is still powered off here, so exclude it or
	// ApplyConfig blocks until deviceApplyConfigTimeout waiting for a fetch
	// confirmation that can never come.
	cluster.ApplyConfig(clusterConfig, true, true, devName[2])
