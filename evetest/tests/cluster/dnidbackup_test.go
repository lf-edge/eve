...
//     evetest.ReplaceVolumeConfig: the volume gets a fresh UUID rather than
//     a bumped generation counter -- an alternate method to purge a volume
//     (see VolumeGenerationPolicy). Unlike bumping generation on an
//     already-replicated volume, a brand new volume UUID starts with no
//     replica history, so this does not necessarily need volumemgr's own
//     not-yet-built backup-DNID bypass (see the DNID rework plan) the way
//     resyncing an existing volume would -- whether it holds up in
//     practice is what this step checks.
//     Issued through cluster.PurgeApplication, not
