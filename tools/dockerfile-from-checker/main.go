...
// * another array with directories with build.yml (potentially a linuxkit package)
func gatherDirs(dir string) ([]string, []string) {
	var dockerfileDirs []string
	var buildYmlDirs []string
	...
	err = filepath.Walk(dir, func(p string, info fs.FileInfo, _ error) error {
		...
		if info.Name() == defaultPkgBuildYML {
			pkgName := filepath.Dir(absPath)
			_, ok := ignorePaths[absPath]
			if !ok {
				// path is not on ignore list
				buildYmlDirs = append(buildYmlDirs, pkgName)
			}
		}
		return nil
	})
	...
	return dockerfileDirs, buildYmlDirs
}

func rootFunc(cmd *cobra.Command, args []string) {
	dockerfileDirs, buildYmlDirs := gatherDirs(args[0])

	froms2dockerfile := dockerfileFroms(dockerfileDirs)
	linuxkitPkgs := linuxkitPackageTags(buildYmlDirs)
...
func linuxkitPackageTags(buildYmlDirs []string) []linuxkitPkg {
	...
	var pkgs []pkglib.Pkg
	for _, ymlDir := range buildYmlDirs {
		pkg, err := pkglib.NewFromConfig(pkglibConfig, ymlDir)
		if err != nil {
