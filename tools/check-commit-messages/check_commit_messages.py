...
def check_commit_message(commit):
    ...
    skip_capital_letter_check = False

    # For kernel update commit generated by the script, ignore the capital letter check
    if subject.startswith("Kernel update"):
        skip_capital_letter_check = False

    # Check if the body starts with a capital letter
    if not skip_capital_letter_check and not body_lines[0][0].isupper():
